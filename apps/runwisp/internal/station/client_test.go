...
// process restart, but the station client used to snapshot that registry once
// at startStationClient time. tasks.sync kept reporting the boot-time task set
...
func TestSendProtocolError_DropsOnFullBuffer(t *testing.T) {
	...
	env.client.sessions.sendProtocolError(session, StationErrorKindValidation, "boom", "req-1", "")
}
...
// An unsupported (future) station→daemon message type must be ignored cleanly:
...
// newer station sends that this daemon doesn't implement is not a validation
...
// entire daemon, not just the station session.
...
func TestHandleInboundPayloadAuthResultFailureReturnsError(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, StationErrorKindAuth, ce.Kind)
...
func TestIsHardAuthError(t *testing.T) {
	assert.True(t, isHardAuthError(&StationError{Kind: StationErrorKindAuth}))
	assert.False(t, isHardAuthError(&StationError{Kind: StationErrorKindTransient}))
...
func TestClassifyErrorKind(t *testing.T) {
	tests := []struct {
		name     string
		err      error
		expected StationErrorKind
	}{
		{"auth error", &StationError{Kind: StationErrorKindAuth}, StationErrorKindAuth},
		{"validation error", &StationError{Kind: StationErrorKindValidation}, StationErrorKindValidation},
		{"non-station error defaults to transient", context.DeadlineExceeded, StationErrorKindTransient},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
// TestSnapshotForSyncFoldsInStationServices verifies the tasks.sync snapshot
// includes daemon-supervised services the task manager holds (notably
// station-declared "station-<id>" services registered at runtime via service:apply),
// which the TOML localTasks set does not carry. Run-to-completion tasks the
// manager holds (e.g. ad-hoc inline station runs) are not folded in.
func TestSnapshotForSyncFoldsInStationServices(t *testing.T) {
	...
	if _, ok := snapshot["station-01HZX"]; !ok {
		t.Fatalf("snapshot missing station service: %v", snapshotNames(snapshot))
	}

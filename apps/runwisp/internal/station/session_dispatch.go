...
func (sr *sessionRunner) run(ctx context.Context, session *wsSession) error {
	...
	if status == websocket.StatusNormalClosure || status == websocket.StatusGoingAway {
		...
		// station side looks exactly like this.
		slog.Warn("station closed the session", "status", status.String(), "detail", firstErr.Error())
		return nil
	}
...
func (sr *sessionRunner) handleInboundPayload(ctx context.Context, session *wsSession, payload []byte) (err error) {
	defer func() {
		if rec := recover(); rec != nil {
			slog.Error("recovered from panic handling inbound station message",
				"err", rec, "stack", string(debug.Stack()))
			sr.sendProtocolError(session, StationErrorKindValidation, "message could not be processed", "", "")
	...
	if err != nil {
		if errors.Is(err, ErrUnsupportedMessageType) {
			// Forward-compat: station sent a message type this daemon version
			...
			// station-side WARN + diagnostic event for every such frame. See the
		...
		sr.sendProtocolError(session, StationErrorKindValidation, "Failed to parse message", "", "")
		return err
	}
	...
	case protocol.ProtocolErrorMessage:
		slog.Info("station protocol error", "code", message.Code, "message", message.Message, "requestId", message.RequestID, "executionId", message.ExecutionID)
		return nil
...
// read-loop goroutine would take down the whole daemon, not just the station
...
func (sr *sessionRunner) handleExecutionDispatch(ctx context.Context, session *wsSession, message protocol.ExecutionDispatchMessage) error {
	if message.Execution == nil {
		sr.sendProtocolError(session, StationErrorKindValidation, "execution is required", "", "")
		return nil
	}
...
func (sr *sessionRunner) sendProtocolError(session *wsSession, kind StationErrorKind, message, requestID, executionID string) {

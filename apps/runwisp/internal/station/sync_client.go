...
type syncTask struct {
	...
	// by TaskParam.Key (the wire shape station's SyncTaskParamDTO expects has no
	...
	// Service-only fields. All omitempty so a station that predates service sync
...
// buildOneSyncTask converts a single model.Task into its station sync representation.
...
func classifySyncHTTPError(statusCode int) StationErrorKind {
	...
	if statusCode >= 400 && statusCode < 500 {
		return StationErrorKindValidation
	}

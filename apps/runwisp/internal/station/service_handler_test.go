...
// station-created service is gated by allow_station_dispatch just like
...
// creation succeeds once the operator opts in via allow_station_dispatch.
...
func TestHandleServiceControl_RoutesActions(t *testing.T) {
	...
	for _, tc := range cases {
		...
		require.NoError(t, err)
		tc.assert(t, runner, sanitizeStationTaskName("svc-1"))
	}
}
...
func TestHandleServiceApply_NameFallbackAndDefaults(t *testing.T) {
	...
	require.Len(t, runner.upserted, 1)
	task := runner.upserted[0]
	assert.Equal(t, "station-Fallback_Svc", task.Name)
	assert.Equal(t, 1, task.Instances)
	assert.Equal(t, "kill", task.LogOnFull)
}

func TestHandleServiceApply_DefaultsToStationServiceName(t *testing.T) {
	h := newDispatchHandler(shellAvailable(), nil)
	runner := h.taskManager.(*fakeTaskRunner)
	...
	require.NoError(t, err)
	require.Len(t, runner.upserted, 1)
	assert.Equal(t, "station-service", runner.upserted[0].Name)
}
...
// The station's Script field is non-omitempty on the wire, so a synced service
...
// service while allow_station_dispatch is off. env can change what a disk-defined
...
// payload carried a script — and the station sends script: null for synced TOML
...
// knobs are what the station re-asserts on every reconnect, and gating those would
...
// bare-name lookup as a station-declared service (both are Kind == KindService

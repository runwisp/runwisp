...
func TestLoadConfigDisabledWhenNoToken(t *testing.T) {
	t.Setenv("RUNWISP_STATION_TOKEN", "")
...
func TestLoadConfigEnabledWithTokenOverride(t *testing.T) {
	t.Setenv("RUNWISP_STATION_URL", "")

	cfg, err := LoadConfig("1.0.0", "rt_token", "", "fp")
	require.NoError(t, err)
	assert.True(t, cfg.Enabled)
	assert.Equal(t, "rt_token", cfg.StationToken)
	assert.Equal(t, defaultStationURL, cfg.BaseURL.String())
}

func TestLoadConfigTokenFromEnv(t *testing.T) {
	t.Setenv("RUNWISP_STATION_TOKEN", "station_token")
	t.Setenv("RUNWISP_STATION_URL", "")
	t.Setenv("RUNWISP_STATION_ALLOW_INSECURE", "")

	cfg, err := LoadConfig("1.0.0", "", "", "fp")
	require.NoError(t, err)
	assert.True(t, cfg.Enabled)
	assert.Equal(t, "station_token", cfg.StationToken)
}
...
func TestLoadConfigHTTPWithAllowInsecure(t *testing.T) {
	t.Setenv("RUNWISP_STATION_ALLOW_INSECURE", "true")
...
func TestLoadConfigURLFromEnv(t *testing.T) {
	t.Setenv("RUNWISP_STATION_TOKEN", "")
	t.Setenv("RUNWISP_STATION_URL", "https://custom.example.com")

	cfg, err := LoadConfig("1.0.0", "tok", "", "fp")
	require.NoError(t, err)
...
func TestLoadConfigURLOverrideTakesPrecedenceOverEnv(t *testing.T) {
	t.Setenv("RUNWISP_STATION_URL", "https://env.example.com")

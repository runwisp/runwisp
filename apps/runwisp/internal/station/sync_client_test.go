...
func TestClassifySyncHTTPError(t *testing.T) {
	tests := []struct {
		...
		{http.StatusUnauthorized, StationErrorKindValidation},
		{http.StatusForbidden, StationErrorKindValidation},
		{http.StatusBadRequest, StationErrorKindValidation},
		{http.StatusUnprocessableEntity, StationErrorKindValidation},
		{http.StatusNotFound, StationErrorKindValidation},
		{http.StatusInternalServerError, StationErrorKindTransient},
		{http.StatusServiceUnavailable, StationErrorKindTransient},
		{502, StationErrorKindTransient},
	}
	for _, tc := range tests {
...
func TestBuildOneSyncTask(t *testing.T) {
	...
	t.Run("service emits service fields and no schedule", func(t *testing.T) {
		...
		assert.Equal(t, string(model.BackoffExponential), st.RestartBackoff)
	...
	t.Run("task with all optional fields", func(t *testing.T) {
		...
		assert.Equal(t, string(model.BackoffExponential), st.RetryBackoff)
...
func TestSyncTasksHTTPErrorBubblesUpAsStationError(t *testing.T) {
...
func TestSyncTasksTransportFailureIsTransient(t *testing.T) {
	// Unreachable URL — net.Dial fails, surfaced as a transient StationError.

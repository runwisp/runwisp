...
func TestCoalesce_ID_DelegatesToInner(t *testing.T) {
	...
	c := New(inner, Config{Window: time.Hour, CoalesceLimit: 10}, testutil.NewFakeClock(time.Unix(0, 0)), nil, nil)
...
func TestCoalesce_FirstEventForwarded(t *testing.T) {
	...
	c := New(inner, Config{Window: time.Hour, CoalesceLimit: 10}, clock, nil, nil)
	defer c.Close(context.Background())
...
func TestCoalesce_RepeatsSuppressedWithinWindow(t *testing.T) {
	inner := testutil.NewFakeChannel("slack-ops")
	clock := testutil.NewFakeClock(time.Unix(0, 0))
	c := New(inner, Config{Window: time.Hour, CoalesceLimit: 100}, clock, nil, nil)
	defer c.Close(context.Background())
...
// TestCoalesce_CoalesceLimitTriggersSummary verifies the Nth suppressed event in a
// window is forwarded with coalesced_count metadata. With CoalesceLimit=3, we
...
func TestCoalesce_CoalesceLimitTriggersSummary(t *testing.T) {
	inner := testutil.NewFakeChannel("slack-ops")
	clock := testutil.NewFakeClock(time.Unix(0, 0))
	c := New(inner, Config{Window: time.Hour, CoalesceLimit: 3}, clock, nil, nil)
	defer c.Close(context.Background())
...
func TestCoalesce_ExpiryWithPendingLogsWarning(t *testing.T) {
	...
	c := New(inner, Config{Window: time.Hour, CoalesceLimit: 100}, clock, logger, nil)
	defer c.Close(context.Background())
...
func TestCoalesce_WindowCloseFailureReportsInApp(t *testing.T) {
	...
	clock := testutil.NewFakeClock(time.Unix(0, 0))
	c := New(inner, Config{Window: time.Hour, CoalesceLimit: 1000}, clock, nil, sink)
	defer c.Close(context.Background())
	mt := withManualTimers(c)
...
func TestTimerFlush_EmptyState(t *testing.T) {
	inner := testutil.NewFakeChannel("slack-ops")
	c := New(inner, Config{Window: time.Hour, CoalesceLimit: 1}, nil, nil, nil)
	defer c.Close(context.Background())

...
// verify that stationTaskRunner delegates each method correctly.
...
func TestStationTaskRunner_GetTask_DelegatesAndPropagates(t *testing.T) {
	inner := &stubTaskRunner{
		getTaskOut: &model.Task{Name: "backup"},
		getTaskOK:  true,
	}
	a := &stationTaskRunner{stationRuntime: inner}
...
func TestStationTaskRunner_UpsertTask_DelegatesTask(t *testing.T) {
	inner := &stubTaskRunner{}
	a := &stationTaskRunner{stationRuntime: inner}
...
func TestStationTaskRunner_TriggerStationRun_TagsTriggeredByStation(t *testing.T) {
	want := &model.Run{ID: "r1", TaskName: "alpha", CreatedAt: time.Now()}
	inner := &stubTaskRunner{triggerRun: want}
	a := &stationTaskRunner{stationRuntime: inner}

	got, err := a.TriggerStationRun("alpha", "exec-123", map[string]string{"REGION": "eu"})
	require.NoError(t, err)
	assert.Same(t, want, got)
	assert.Equal(t, "alpha", inner.gotTriggerTask)
	assert.Equal(t, model.TriggeredByStation, inner.gotTrigger.TriggeredBy)
	assert.Equal(t, "exec-123", inner.gotTrigger.ExecutionID)
	// The station protocol carries a plain map; the adapter lifts it into the
...
func TestStationTaskRunner_TriggerStationRun_PropagatesError(t *testing.T) {
	boom := errors.New("trigger failed")
	inner := &stubTaskRunner{triggerErr: boom}
	a := &stationTaskRunner{stationRuntime: inner}

	_, err := a.TriggerStationRun("x", "e", nil)
	assert.ErrorIs(t, err, boom)
}

func TestStationTaskRunner_TerminateRunByExecutionID_Delegates(t *testing.T) {
	inner := &stubTaskRunner{}
	a := &stationTaskRunner{stationRuntime: inner}
...
func TestStationTaskRunner_TerminateRunByExecutionID_PropagatesError(t *testing.T) {

...
| `manual_trigger` | `true`       | Allow stop/restart/start from the CLI, API, Web UI, TUI, and cloud control plane. Set `false` to lock the service to its restart policy until you edit the TOML and reload. |
...
API, and the cloud control plane all refuse it; only a `runwisp.toml`

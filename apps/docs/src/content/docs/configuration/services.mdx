...
| `manual_trigger` | `true`       | Allow stop/restart/start from the CLI, API, Web UI, TUI, and station control plane. Set `false` to lock the service to its restart policy until you edit the TOML and reload. |

Set `manual_trigger = false` if you don't want anyone stopping or
restarting this service by hand, from any front door. The **Stop
Service** button, TUI `s`/`r`, `runwisp service stop/restart`, the REST
API, and the station control plane all refuse it; only a `runwisp.toml`
edit plus `runwisp reload` changes its running state.
...
| Key                | Default         | What it does                                                                                                                                                                                                                                                  |
| ------------------ | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `restart`          | `"always"`      | When to restart an exited instance: `always`, `on_failure`, or `never`. Most services want the default.                                                                                                                                                       |
...
| `restart_attempts` | `3`             | Consecutive failed starts an instance tolerates before it's marked **FATAL** and stops restarting. `0` means give up after the very first failed start.                                                                                                       |

An explicit `0` (or `"0s"`) on any of these three keys is kept exactly as
written — it is never silently replaced by the default. Only omitting the
key falls back to `[defaults]` and then the built-in.
...
These delays throttle _automatic_ restarts after a process exits on its
own. When you hit **Restart** (or stop a single instance) by hand, the
instance comes back immediately — the backoff only exists to keep a
crash loop from hammering, not to slow down a deliberate action.

The default, `restart = "always"`, is the deal with a service: it comes
back no matter how it exited. Narrow it to `"on_failure"` if a clean exit
should retire the instance instead of relaunching it, or `"never"` if you
want the restart machinery (delay, backoff, FATAL) without the daemon
ever respawning it on its own. If what you want is "run once and exit,"
that's a [task](/concepts/tasks-vs-services/) instead.
...
([what counts as failure](/configuration/tasks/#retries--timeout) — a clean exit
on a success code never does), and that's a failed start. Rack up more
than `restart_attempts` failed starts in a row and the instance goes
...
fresh `restart_attempts` budget and tries again; a restart is your "try
...
no `max_concurrent` either, for the same reason.
...
| Key      | Default  | What it does                                                                                                          |
| -------- | -------- | --------------------------------------------------------------------------------------------------------------------- |
| `notify` | _(none)_ | Notifier IDs to alert when an instance ends `failed`, `crashed`, `timeout`, or `missed`, or when a slot goes `fatal`. |

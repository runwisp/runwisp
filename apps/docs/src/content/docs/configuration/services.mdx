...
| `backoff_reset_after` | duration | inherited       | Replica must stay up at least this long before its restart counter resets. Inherits from `[defaults] backoff_reset_after` (default `"60s"`). |
...
up for `backoff_reset_after` resets its backoff counter, so transient
flapping doesn't permanently slow restarts on a service that eventually
stabilises.
...
have `max_concurrent` — replica count is governed by `instances`, not
in-flight overlap.
...
| Key             | Type     | Default | What it does                                                                                           |
| --------------- | -------- | ------- | ------------------------------------------------------------------------------------------------------ |
| `graceful_stop` | duration | `"5s"`  | SIGTERM grace period per replica before SIGKILL — for manual stop, `Restart Service`, daemon shutdown. |

`graceful_stop` is process-group-wide: SIGTERM goes to the replica's
...
during whole-daemon shutdown each replica is bounded by the daemon
cap regardless of its own setting.
...
- `max_concurrent`, `queue_max` — replica count is `instances`; services don't queue.

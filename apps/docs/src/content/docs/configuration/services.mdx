...
| `manual_trigger` | `true`       | Allow manual trigger from CLI / API / UI. (Restart is the usual interaction for services.) |
...
- `max_concurrent`, `max_queued` — instance count is `instances`; services don't queue.

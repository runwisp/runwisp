...
| `env`                 | _(none)_     | Inline KEY/VALUE map merged into every task and service. Task-level entries override on key collision. See [`[tasks.*]` Environment](/configuration/tasks/#environment). |
| `env_file`            | _(none)_     | Path to a dotenv file whose KEY=VALUE pairs are merged into every task and service. Visibility rules match per-task `env_file`.                                          |

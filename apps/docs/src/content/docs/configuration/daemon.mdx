...
| `reveal_vars`      | _empty_ | Names of variables whose resolved `${...}` value may be shown in the REST API and Web UI. Everything not listed stays hidden. See [Variables & secrets](/configuration/variables/).                                             |
...
`${...}` placeholders work in [every `runwisp.toml`
field](/configuration/variables/), and anything RunWisp pulled out of a
variable is **hidden by default** — the API and Web UI show the raw
`${NAME}` placeholder, not the value. `reveal_vars` is the opt-in
override: list the variable names whose resolved value is safe to
display, and only those show through.
...
The list holds **variable names**, declared once — not per use site. A
name you forget to add just keeps showing as `${NAME}`, so the failure
mode is "too hidden", never "leaked". `${file:...}` values can never be
revealed; a file is the always-secret channel. The full model — what's
hidden, what's shown automatically, and how it's redacted from logs —
is on the [Variables & secrets](/configuration/variables/) page.

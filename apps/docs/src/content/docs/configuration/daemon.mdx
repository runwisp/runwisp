...
A glob only picks up the files cron itself would schedule: regular files
whose names are made of letters, digits, `-` and `_`. That rule is how
`/etc/cron.d/backup.dpkg-old`, a hand-renamed `job.disabled`, and a
`README` stay out of your schedule — so RunWisp skips them too, and says

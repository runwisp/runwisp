...
`/api/info` (and the UI) so you know a restart will pick up changes.
Like all config, includes are read once at boot;
[reload is restart-only](/configuration/overview/).
...
Two more global flags — `--log-level` and `--log-format` — shape the
daemon's own log output; [Operations: Logging](/operations/logging/)
covers them.

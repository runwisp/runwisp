...
RunWisp warns at boot if cron looks live, whether that's a process running
right now or a systemd unit that's merely enabled (stopped this boot, back
on the next one), and whether your `include_cron` reads `/etc/cron.d` or
only per-user spool files. It can't stop cron for you either way.
:::

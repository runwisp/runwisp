...
| `env_file` | _(none)_ | Path to a dotenv file (`KEY=VALUE` per line). Path is visible, but its keys and values stay on disk. |

When the task starts, the values from `env` and `env_file` get merged on
top of the daemon's own environment. On a key collision, later wins:

1. The daemon process's own environment.
2. `[defaults] env_file`, then `[defaults.env]`.
3. The task's `env_file`, then the task's `env`.

An `env_file` path can be absolute or relative. Relative paths resolve
against the directory `runwisp.toml` lives in — the same root the config
loader uses for every other file lookup. The format is plain dotenv:
`KEY=VALUE` per line, `#` starts a comment, blank lines are skipped, and
there's no shell expansion or interpolation.

**So why two knobs?** Anything in inline `env` is treated as out in the
open — it sits right next to `cron` and `run` in the dashboard, the TUI,
and `GET /api/tasks`. Anything that should _not_ be on display — API
tokens, signing keys — belongs in `env_file`, whose contents never leave
disk. The API will show you the _path_ so you can tell where a value
came from, but never the keys or the values themselves.

```toml
[tasks.backup]
cron     = "0 3 * * *"
run      = "/usr/local/bin/backup.sh"
env_file = "/etc/runwisp/backup-secrets.env"
...
bytes, no single value can top 32 KiB, and the inline + file entries for
...
block. Whatever's listed in `[notify] append_notifiers` (default
`["inapp"]`) gets tacked on automatically — set `append_notifiers = []`

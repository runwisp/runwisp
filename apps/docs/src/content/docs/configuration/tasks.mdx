...
Values from `env` and `env_file` are merged into the daemon's own
environment when the task starts. Precedence (later wins on key
collision):
...
`env_file` paths can be absolute or relative. Relative paths are
resolved against the directory of `runwisp.toml` (the same root the
config loader uses for any other file lookup). The format is standard
dotenv: `KEY=VALUE` per line, `#` introduces a comment, blank lines are
ignored, no shell expansion or interpolation.

**Why two knobs?** Anything in inline `env` is treated as openly
visible — it shows up alongside `cron` and `run` in the dashboard, the
TUI, and `GET /api/tasks`. Anything that should _not_ be visible (API
tokens, signing keys) goes in `env_file`, where the file's contents
never leave disk: the API exposes the path so operators can see where a
value came from, but never the keys or values themselves.
...
Validation is enforced at config load: keys must match
`^[A-Za-z_][A-Za-z0-9_]*$`, values cannot contain NUL bytes, no single
value exceeds 32 KiB, and the combined count of inline + file entries
per task is capped at 256.

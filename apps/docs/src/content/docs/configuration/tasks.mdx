...
| Key           | Default    | What it does                                                                                                                                      |
| ------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `working_dir` | _(daemon)_ | Directory the process runs in. Relative paths resolve against the `runwisp.toml` directory; `~` expands. Must exist and be a folder.              |
...
brace expansion). Both are validated at config load — a missing directory
or a relative shell path fails loudly up front. `shell` can also be set

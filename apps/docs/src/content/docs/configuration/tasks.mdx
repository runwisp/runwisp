...
command that's expected to return non-zero, `exit_codes` above is the
better tool.
...
| `treat_missed_as_failure` | `true`   | Whether a [missed scheduled run](/concepts/scheduling/#alerting-on-missed-runs) alerts. It rides `notify_on_failure`'s channels automatically; set `false` to silence the alert (the `missed` run row is still recorded). |

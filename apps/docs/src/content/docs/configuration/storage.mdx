...
plus their `.idx`, `.tidx`, and `.meta` sidecars across every task's log
...
- **During every run, as the logs grow.** If free space dips below the
  threshold mid-run, the log writer stops accepting new lines and the
  daemon raises a `log.disk_pressure` notification (severity `warn`) so
  you find out about the output you're no longer seeing. What happens to
...
The starter file ships with `5gb` / `500mb`, which is a safe baseline
for a typical VPS. Tune it to your setup:

...
token, that sort of thing. There are three ways to hand it over:

- **Env var** (the one to reach for): set
  `webhook_url_env = "RUNWISP_SLACK_URL"` and put the actual value in
  your shell or systemd unit. Works the same way everywhere.
- **File**: set `webhook_url_file = "secrets/slack.url"`. Relative paths
  resolve under the data directory. Handy when a secrets manager drops
  the value on disk for you — just `chmod 600` the file.
...
Pick **exactly one** of the three per secret. Set two and it's a
config-load error — the loader won't try to guess which you meant, it

...
confused with the daemon **fingerprint** — see the
[glossary](/concepts/glossary/#fingerprint) — those are unrelated.)
...
- [Per-task notification sugar](/notifications/per-task/) — exactly
  how `notify_on_failure` and `notify_on_success` expand.

...
[Notification sugar](/notifications/per-task/) for the exact
...
`append_notifiers` includes `"inapp"`). A failure of `metrics-export`
triggers only the Slack delivery — the in-app catch-all from
`append_notifiers` lights up the bell on top of that.
...
- [Notification sugar](/notifications/per-task/) — the shortcut for the common case (works on `[tasks.*]` and `[services.*]`).

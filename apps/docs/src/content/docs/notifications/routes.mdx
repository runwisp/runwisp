...
the bell** — never back through the outbound router. You can still send
delivery failures somewhere _else_, though, because the router accepts
`notify.delivery_failed` as a valid `match.kind`:
...
`match.kind` values that aren't one of the eight listed above;

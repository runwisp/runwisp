...
| ---------------- | -------- | ---------------------------------------------------------------------------- |
| `match.kinds`    | no       | Event kinds this rule applies to. Omit (or leave empty) to match all kinds.  |
...
| `run.stopped`            | A run is cancelled (manual stop, or `on_overlap = "kill"`).                                      |
...
| `notify.delivery_failed` | A notifier exhausted its retries. Delivered only to the bell; cannot be used as a `match.kinds`. |
...
`match.kinds` values that aren't one of the valid kinds listed above;

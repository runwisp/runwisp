...
There's no `treat_missed_as_failure = [...]` channel list, and that's
deliberate. A [missed scheduled run](/concepts/scheduling/#alerting-on-missed-runs) —
...
What you _can_ do is turn it off for a task that's expected to miss
ticks — a laptop job that only runs while you're at your desk, say:
...
notify_on_failure = ["slack-ops"]
treat_missed_as_failure  = false   # failures still page; missed ticks stay quiet
```

`treat_missed_as_failure` is a boolean, not a channel list. It defaults to

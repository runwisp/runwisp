...
`id`, `type`, and `webhook_url` are required. Keep the real URL out of
the file with `${...}` interpolation —
[storing the secret](/notifications/model/#storing-the-secret).

| Key             | Required | What it does                                                                        |
| --------------- | -------- | ----------------------------------------------------------------------------------- |
| `webhook_url`   | yes      | The webhook URL. Use `${VAR}` or `${file:/path}` to keep the value out of the file. |
...
Put a `${...}` placeholder in `webhook_url` and let the daemon fill it
in at startup. Pick whichever source fits your deployment.
...
<Tabs syncKey="slack-secret">
    ...
    <TabItem label="File on disk" icon="document">
        ...
        id          = "slack-ops"
        type        = "slack"
        webhook_url = "${file:/home/you/.config/runwisp/slack-ops.url}"
        channel     = "#ops"
        ```
    </TabItem>
    <TabItem label="Inline (don't)" icon="warning">
        A value with no `${` is taken literally, so you _can_ paste
        the URL straight in. Avoid it — config files are often
        committed to git or shared in chat, and an inline URL will
        leak.

...
The id (as the table key), `type`, and `webhook_url` are required. Use
...
options, including inline channel overrides and routing a success ping.
...
No [`external_url`](/configuration/daemon/#external_url)? The action button drops off. No log file? The

...
The id in the table header, plus `type` and `webhook_url`, are required. Use
...
channel, create a second webhook and a second `[notifiers.*]` entry.
...
of options, including routing a success ping with a `[[route]]`.
...
notifiers = ["discord-ops", "discord-oncall"]
```
...
```bash
runwisp run smoke-test       # whatever you have that exits non-zero
```
...
No [`external_url`](/configuration/daemon/#external_url)? The title just isn't a link. No log file? The

...
also be set — [the secret rule](/notifications/model/#storing-the-secret).
...
| `password`        | one-of (see below) | Inline password. Avoid in committed configs.                                       |
| `password_env`    | one-of             | Name of an env var holding the password.                                           |
| `password_file`   | one-of             | Path to a file containing the password. Relative paths resolve under the data dir. |
...
<Tabs syncKey="smtp-provider">
    ...
    <TabItem label="SendGrid" icon="seti:shell">
        ...
        username     = "apikey"
        password_env = "RUNWISP_SENDGRID_API_KEY"
        ```
    </TabItem>
    <TabItem label="Amazon SES" icon="seti:shell">
        ...
        to           = ["ops@yourdomain.com"]
        username     = "AKIA..."
        password_env = "RUNWISP_SES_SMTP_PASSWORD"
        ```
    </TabItem>
    <TabItem label="Mailgun / Postmark" icon="seti:shell">
        ...
        password_env = "RUNWISP_MAILGUN_PASSWORD"
        ```
        ...
        password_env = "RUNWISP_POSTMARK_TOKEN"
        ```
    </TabItem>
...
<Tabs syncKey="smtp-secret">
    ...
    <TabItem label="File on disk" icon="document">
        ...
        to            = ["ops@example.com"]
        username      = "apikey"
        password_file = "/home/you/.config/runwisp/smtp.password"
        ```
    </TabItem>
...
- Two of `password`, `password_env`, `password_file` set at the
  same time.
- `username` set without any password source — or a password
  source set without `username`. Either both or neither.

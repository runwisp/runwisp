...
| `tls`             | no       | `"starttls"` (default for 25/587), `"implicit"` (default for 465), `"off"`.  |
...
<Tabs syncKey="smtp-provider">
    ...
    <TabItem label="Local Postfix" icon="seti:shell">
        ...
        so `tls = "off"` only works alongside an auth-less relay.
...
- `tls = "off"` combined with credentials — RunWisp refuses to

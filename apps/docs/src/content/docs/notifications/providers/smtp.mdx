...
The id (as the table key), `type`, `host`, `from`, and at least one `to` are required.
...
| `tls_mode`        | no       | `"starttls"` (default for 25/587), `"implicit"` (default for 465), `"off"`.  |
...
<Tabs syncKey="smtp-provider">
    ...
    <TabItem label="Local Postfix" icon="seti:shell">
        ...
        so `tls_mode = "off"` only works alongside an auth-less relay.
...
```toml
[tasks.critical-job]
notify = ["email-ops", "slack-ops", "tg-oncall"]
```
...
- **Common causes:** wrong port for the chosen `tls_mode` (Gmail
  needs `587` + STARTTLS; SES on `465` needs `tls_mode = "implicit"`),
...
- The **View run** link points at
  `<[`external_url`](/configuration/daemon/#external_url)>/tasks/<task>/<id>`, taken from
  [`external_url`](/configuration/daemon/#external_url).
  When [`external_url`](/configuration/daemon/#external_url) is unset the link is omitted — no broken
  anchors.
...
- `tls_mode = "off"` combined with credentials — RunWisp refuses to
...
<CardGrid>
    <LinkCard
        ...
        description="Skip the relay config and use the sendmail binary already on the box."
    />
    <LinkCard
        title="Slack"

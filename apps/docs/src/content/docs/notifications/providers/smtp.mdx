...
internal MTA, etc.). When `username` is set, `password` must be too.
Keep the real password out of the file with `${...}` interpolation —
...
| `password`        | with username | The password. Use `${VAR}` or `${file:/path}` to keep the value out of the file. |
...
Put a `${...}` placeholder in `password` and let the daemon fill it in
at startup. Pick whichever source fits your deployment.
...
<Tabs syncKey="smtp-secret">
    ...
    <TabItem label="File on disk" icon="document">
        ...
        to       = ["ops@example.com"]
        username = "apikey"
        password = "${file:/home/you/.config/runwisp/smtp.password}"
        ```
    </TabItem>
    <TabItem label="Inline (don't)" icon="warning">
        A value with no `${` is taken literally, so you _can_ paste
        the password straight in. Avoid it — config files are often
        committed to git or shared in chat.
...
- A `${VAR}` whose env var is unset at daemon start (with no
  `:-default`) — the daemon refuses to start and names the variable.

...
`id`, `type`, `bot_token`, and `chat_id` are required. Keep the real
token out of the file with `${...}` interpolation —
[storing the secret](/notifications/model/#storing-the-secret).

| Key             | Required | What it does                                                                      |
| --------------- | -------- | --------------------------------------------------------------------------------- |
| `bot_token`     | yes      | The bot token. Use `${VAR}` or `${file:/path}` to keep the value out of the file. |
...
Put a `${...}` placeholder in `bot_token` and let the daemon fill it in
...
<Tabs syncKey="telegram-secret">
    ...
    <TabItem label="File on disk" icon="document">
        ...
        id        = "tg-oncall"
        type      = "telegram"
        bot_token = "${file:/home/you/.config/runwisp/tg-oncall.token}"
        chat_id   = "-1001234567890"
        ```
    </TabItem>
    <TabItem label="Inline (don't)" icon="warning">
        A value with no `${` is taken literally, so you _can_ paste
        the token straight in. Avoid it — config files are often

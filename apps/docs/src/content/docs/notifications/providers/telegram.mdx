...
The id (as the table key), `type`, `bot_token`, and `chat_id` are required. Use
...
- The **View run** link points at
  `<[`external_url`](/configuration/daemon/#external_url)>/tasks/<task>/<id>`, taking from
  [`external_url`](/configuration/daemon/#external_url). When
  [`external_url`](/configuration/daemon/#external_url) is unset the link line is omitted — no broken anchors.

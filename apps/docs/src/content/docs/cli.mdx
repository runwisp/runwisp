...
✓ runwisp.toml is valid
  3 tasks, 2 services, 2 notifiers, 1 route
  scheduler timezone: Europe/Bratislava (config)
```

On failure it prints the offending key, value, and a hint when one
applies:

```
✗ runwisp.toml: 2 errors

  [tasks.health-probe] parallelism = 2
    unknown key 'parallelism' — did you mean 'max_concurrent'?

  [tasks.bulky-job] keep_runs = "unlimited"
    keep_runs must be a positive integer; the "unlimited" keyword is
    no longer accepted. Omit the field to disable the cap.
```

Pair it with a pre-commit hook or CI job:

...
If any tasks came from a crontab a live cron daemon still owns, the
header carries a chip reading ``⏸ N held by cron — `sudo runwisp takeover` ``.
See [Held jobs](/coming-from/cron/).
...
then lists your tasks and services grouped by [`group`](/configuration/tasks/#group).
...
- **Task detail** (a task or service selected from the sidebar) —
  ...
  history for that one task. A [held](/coming-from/cron/) task
  shows `⏸ held — cron still owns this job` on its schedule line and no
  "Next:" time, because RunWisp isn't the one firing it — **Run Now**
  still works. **Run Now** asks you to confirm before it
  fires. If the task declares [parameters](/concepts/parameters/),
  **Run Now** opens a form instead — fill in the values, then submit to
  trigger with those inputs.
...
succeeded / failed); while a filter is active, a banner under the column
...
- **The header status badge** carries the end reason — `failed`,
  [`timeout`](/configuration/tasks/#timeout), `crashed`, `stopped`, `skipped`,
...
"skip"` rejected it), `-2` is a `crashed` run the daemon was killed
  during. A [`timeout`](/configuration/tasks/#timeout) shows whatever exit the OS produced when the
...
attempt 1 `succeeded`. Read attempt 0's log to find out why the first
try failed even when the retry recovered; that's the bug-hunting use
of [`keep_runs`](/configuration/tasks/#keep_runs).

If a run never ran at all (`skipped` because of [`on_overlap`](/configuration/tasks/#on_overlap),
`stopped` because of `on_overlap = "kill"`), the row is still
...
A daemon started with [`RUNWISP_AUTH=off`](/operations/auth/#running-without-a-password)

...
- **Task overview** — filter tabs (All · Attention · Running ·
  ...
  care about without scrolling. A task read from a crontab a live cron
  daemon still owns carries a `held` badge here, with the full
  explanation in its tooltip — see
...
- **Status** — five plain-language buckets: **Running**, **Succeeded**,
  **Failed**, **Skipped**, and **Stopped**. Tick **Failed** to catch
  ...
  anything fail?". Need to be surgical (say, only [`timeout`](/configuration/tasks/#timeout))? Open
...
- **Triggered by** — scheduled (cron), the REST API, the Web UI or TUI's
  **Run Now**, the `runwisp run` CLI, a service auto-start, or a run on
  daemon start.
...
If the task declares [parameters](/concepts/parameters/), clicking
**Run Task** opens a form — each declared input renders as a text field,
dropdown, or toggle, with validation and descriptions right there. Fill
in what you need and submit. Tasks with no [`params`](/configuration/tasks/#params) trigger instantly
with no form.
...
[`timeout`](/configuration/tasks/#timeout), `crashed`, `stopped`, `skipped`, or [`log_overflow`](/concepts/logs/#rotation-log_max_size-and-log_on_full) —

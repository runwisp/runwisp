...
RunWisp doesn't watch `runwisp.toml` for changes — edits take
effect on the next boot. When you've changed the file, run
`runwisp restart`: it stops the daemon and starts a fresh one with
your edits loaded. (If you installed RunWisp as a service, it asks
systemd or launchd to do the restart so the service manager stays
in the loop.) Until you do, the TUI header, `runwisp status`, and
the Web UI all show a notice that the config on disk no longer
matches what the daemon is running. `runwisp stop` shuts the
daemon down without starting a new one.

...
That's the plain case. If RunWisp also finds a `docker-compose.yml`
next to you, or a crontab it can read, the prompt says so and the
starter it writes wires that in too. Either way, answering with
anything but yes leaves your directory untouched and tells you how to
write `runwisp.toml` yourself.

On a root Linux box with systemd and cron actually running, that
crontab case becomes the whole cutover in one question:
...
[Take over from cron](/replacing-cron/take-over-from-cron/) covers when
you get that prompt and what to do when you don't.

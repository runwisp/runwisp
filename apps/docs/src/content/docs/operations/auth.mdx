...
like `runwisp`, `runwisp tui`, `runwisp list`, and `runwisp run`
all connect through the socket automatically.

Point `runwisp tui` or `runwisp run` at a `--url` (or set
...
:::tip[Installing as a service does this for you]
A fresh daemon with no `RUNWISP_PASSWORD` mints a new random password
every boot — fine for a throwaway dev daemon, but it means you can never
stay logged in to a long-lived one. So
[`runwisp service install`](/operations/autostart/#a-stable-web-ui-password)
generates a stable password for you: it writes one into a `0600` drop-in
beside the unit, restarts to apply it, and prints it once. Re-installing
never rotates it. Set `RUNWISP_PASSWORD` yourself and the installer leaves
it alone.
:::
...
Set `RUNWISP_AUTH=off` (or `true`) and every `/api/` route answers
...
shows `Password disabled (RUNWISP_AUTH=off)` for the same reason, and
...
- `RUNWISP_AUTH=off` and `RUNWISP_PASSWORD` are mutually exclusive —
...
- The local Unix socket and the optional station connection are
  unaffected; this switch only opens the local HTTP API and Web UI.

### Docker for local dev

This is the headline use case. The official `runwisp/runwisp` image
already binds `0.0.0.0` and requires you to pick an auth setting
explicitly (see [Docker](/getting-started/docker/) for the full
picture), so for local dev it's just:
...
the proxy own TLS, leave [`tls`](/configuration/daemon/#tls-tls_cert-tls_key) at its default of `"off"` so RunWisp
serves plain HTTP on its private side, and tell the daemon which IP
ranges that proxy will be calling from:

```bash
export RUNWISP_TRUSTED_PROXIES='10.0.0.0/8,2001:db8::/32'
runwisp daemon --host 127.0.0.1 --port 9477
```

`RUNWISP_TRUSTED_PROXIES` is a comma-separated list of CIDR ranges
...
Prefer to keep it in config rather than the environment? The same list
lives in [`trusted_proxies`](/configuration/daemon/#trusted_proxies)
as a TOML array. The env var wins when both are set, so you can pin a
default in `runwisp.toml` and still override it per-deployment.

The daemon refuses `0.0.0.0/0` and `::/0`, whether set by the env var or
the TOML key. Trusting the whole internet would let any random client lie
...
<CardGrid>
    <LinkCard
        ...
        description="Locked out, or logins being refused? Start here."
    />
    <LinkCard
        title="The Web UI tour"
        href="/getting-started/web-ui-tour/"
        description="What you get once you're past the login — runs, tasks, notifications."
    />
    <LinkCard
        title="[daemon]"
        href="/configuration/daemon/"
        description="TLS, the bind address, the control socket, and every env var."
    />
    <LinkCard
        title="Metrics"
        href="/operations/metrics/"
        description="The one endpoint that's public by design, and the trade-off it carries."
    />
</CardGrid>

...
A short list of routes that intentionally bypass the password / JWT
boundary:

- `/health` — liveness probe; always `200 OK`.
- `/metrics` — Prometheus / OpenMetrics scrape endpoint. See
  [Metrics](/operations/metrics/) for the surface and the security
  trade-off.

Everything under `/api/` (other than the auth handshake itself)
requires either a valid session cookie or a request delivered over

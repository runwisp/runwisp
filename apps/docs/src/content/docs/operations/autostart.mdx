...
3. Offers to stop and mask cron, if cron is running and RunWisp is
   reading its crontabs. See [retiring cron](#retiring-cron).
...
An install that [retired cron](#retiring-cron) adds one more line
reporting whether the masked cron unit is still masked:
...
Once your cron jobs are running as RunWisp tasks via
[`include_cron`](/configuration/daemon/#include_cron) or
[`runwisp promote`](/recipes/migrating-from-cron/), leaving cron running
means every job fires twice — once from cron, once from RunWisp. So
`sudo runwisp service install` asks about it:
...
Say yes and it stops and masks the system cron unit (`cron.service`,
`crond.service`, or `cronie.service`, whichever systemd knows about) once
RunWisp is confirmed running. Say no and cron is left exactly as it was.

For scripted installs, `--take-over-cron` answers the question up front
and skips the prompt. `--yes` deliberately does **not** answer it: masking
cron is destructive and not something to imply from a blanket "yes to
everything".

The question is only ever asked when it's live and actionable — a
terminal, root, the system scope, cron actually running, and a config
that's actually reading crontabs. Otherwise nothing is asked. With
`--take-over-cron`, the same conditions become hard refusals, before
anything is written:

- **Nothing is being read.** No `include_cron` pattern matched a
  crontab. Masking cron here would silently stop every job on the box.
- **A cron source failed to load.** Any `runwisp status`/`runwisp
validate` warning about a skipped cron file or job blocks the
  take-over too — pass `--allow-skipped-cron-jobs` to proceed anyway
  (those jobs stay stopped).
- **A cron job would run as a user this daemon can't become.** Not
  overridable — there's no flag that makes a `user =` cron task run
  correctly once cron stops covering it.
- **Root or the system scope is missing.** Retiring cron only makes
  sense for the one system-wide daemon that can execute every
  account's jobs.

If cron _is_ running but one of those refusals applies, the install says
so in a one-line note rather than staying silent — you're about to end up
with two schedulers and should know why RunWisp isn't offering to fix it.

Order matters for the mechanics: cron is stopped and masked only
_after_ RunWisp's unit is written and reloaded, and RunWisp is started
only after cron is confirmed masked. If starting RunWisp then fails,
the installer unmasks cron (and restarts it, if it was running) rather
than leaving the box with no scheduler at all.

`service uninstall` reverses it: it unmasks (and restarts) the cron unit,
but only when this instance's own unit recorded the take-over — a
`# runwisp-masked-cron: <unit>` marker line, next to the existing
config-hash and binary-SHA lines. RunWisp will never unmask a service it
can't prove it masked itself.

`service status` gains a `Cron:` row once that marker exists:

```
  Cron:       cron.service masked by this instance
```

If it instead reads `active — jobs may be firing twice`, something
(an operator, a package upgrade) brought cron back after the take-over
and both schedulers are now running the same jobs.

**Linux only.** macOS's cron (`com.vix.cron`) lives under
SIP-protected `/System/Library/LaunchDaemons` — nothing running as your
user can mask it. There's nothing to offer there, and `--take-over-cron`
refuses with a pointer to `runwisp import cron` followed by removing the
crontab by hand (`crontab -r`).

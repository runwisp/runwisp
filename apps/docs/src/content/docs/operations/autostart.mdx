...
The command never edits `runwisp.toml`. It is OS integration only.
...
`runwisp status` is the _daemon_ health check (is it alive right
now?). `runwisp service status` is the _autostart_ check (will it
...
The unit needs absolute, durable paths for the binary and the data
dir. `runwisp service install` resolves them by these rules:
...
`runwisp service install` never copies `RUNWISP_PASSWORD` into the
...
Without `--purge`, uninstall is symmetric and reversible: it stops
the service, disables autostart, removes the managed unit, runs
`daemon-reload`, and leaves the data dir untouched.

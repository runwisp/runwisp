...
title: Autostart
description: Wire RunWisp into systemd (Linux, WSL) or launchd (macOS) with one command, so the daemon survives a reboot. Covers the system service, per-user installs with --local, retiring cron, status, uninstall, --purge, --print for Ansible/Nix, the auto-generated Web UI password, and the WSL Task Scheduler recipe.
---
...
5. Generates a [stable Web UI password](#a-stable-web-ui-password) into a
   `0600` drop-in and prints it once, unless you set `RUNWISP_PASSWORD`

...
title: Autostart
description: Wire RunWisp into systemd (Linux, WSL) or launchd (macOS) with one command, so the daemon survives a reboot. Covers the system service, per-user installs with --local, retiring cron, status, uninstall, --purge, --print for Ansible/Nix, the auto-generated Web UI password, carrying your RUNWISP_* environment into the unit, and the WSL Task Scheduler recipe.
---
...
host/port) or want a fresh boot to re-fire [`run_on_start`](/configuration/tasks/#run_on_start) and catch-up.
...
5. Captures your [`RUNWISP_*` environment](#carrying-your-environment) into
   a `0600` drop-in, refreshed on every re-install.
6. Generates a [stable Web UI password](#a-stable-web-ui-password) into its
   own `0600` drop-in and prints it once, unless you set `RUNWISP_PASSWORD`
   yourself or one already exists.
...
5. Captures your [`RUNWISP_*` environment](#carrying-your-environment) and,
   if auth is on, a stable Web UI password — same as the system install.
...
a [cron take-over](/coming-from/cron/) does — but systemd
...
An install that [retired cron](/coming-from/cron/)
...
A daemon with no [`RUNWISP_PASSWORD`](/operations/auth/#the-password) set mints
a new random password every boot — so on a long-lived service you could never
stay logged in, and every restart would drop your sessions. `runwisp service
install` fixes that for you, without any manual drop-in editing:

- When auth is on and you haven't set `RUNWISP_PASSWORD`, the installer
  **generates** a password, writes it into a `0600` drop-in beside the unit
  (`/etc/systemd/system/runwisp.service.d/password.conf`), restarts to apply it,
  and prints it once:
    ...
    Store it now. It won't be shown again here, and 'runwisp password' won't
    disclose it — it lives only in that root-owned drop-in. To rotate it, edit or
    delete the file and restart.
    ```

    The secret goes in the drop-in, not the unit itself: the unit at
    `/etc/systemd/system/runwisp.service` is world-readable, the drop-in is not.

- **Re-installing never rotates it.** If the drop-in already exists, or you set
  `RUNWISP_PASSWORD` yourself, the installer leaves the password alone — a
  re-install won't log everyone out.

- **`--print` / `--dry-run`** say the drop-in would be generated, and write
  nothing.

If you'd rather manage the secret yourself — a Docker secret, systemd
`LoadCredential=`, a vault — set `RUNWISP_PASSWORD` in the environment before you
install (or in your own drop-in) and RunWisp won't touch it. The drop-in lives
next to the managed unit but outside the unit file, so `service install` updates
and re-installs never overwrite it.

:::note[launchd]
macOS launchd has no drop-in mechanism, so a `--local` install on macOS doesn't
generate a password — set `RUNWISP_PASSWORD` in the plist's
`EnvironmentVariables`, or leave it and retrieve the ephemeral one with
[`runwisp password`](/operations/auth/#retrieving-the-ephemeral-password).
:::

## Carrying your environment

A systemd unit (or launchd plist) never inherits the shell you ran the
install from. So `RUNWISP_AUTH=off sudo runwisp service install` used to
quietly install a service that ran with auth **on** anyway — the variable
never reached the unit at all.

`service install` now captures every `RUNWISP_*` variable set in your shell —
`RUNWISP_AUTH`, `RUNWISP_TLS`, `RUNWISP_TRUSTED_PROXIES`, `RUNWISP_STATION_TOKEN`,
an operator-supplied `RUNWISP_PASSWORD`, and so on — into a `0600` drop-in
beside the unit (`/etc/systemd/system/runwisp.service.d/runwisp-env.conf`),
then restarts the daemon so it takes effect:

```bash
RUNWISP_AUTH=off sudo runwisp service install
```

Unlike the password drop-in, this one is refreshed on every install — so if
you change a value or drop one from your shell, just re-run the install and
it catches up. The file itself is `0600`, same as the password drop-in, since
it can carry a secret too.

:::note[launchd]
macOS launchd has no drop-in mechanism, so a `--local` install there doesn't
capture your environment either — set the vars yourself in the plist's
...
Add `--local` to render the per-user unit instead. `--print` never prompts and
never touches cron.
...
[Take over from cron](/coming-from/cron/) for the full

...
`restart` is also the answer to the "runwisp.toml has changed since
the daemon started" notice that `runwisp status`, the TUI header,
and the Web UI banner show after you edit the config — the daemon
never hot-reloads, so a restart is how edits take effect.

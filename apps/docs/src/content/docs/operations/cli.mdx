...
A task carries `"staged": true` when its definition lives in the machine-owned
`runwisp.d/imported.toml` that [`import --write`](#migrate-an-existing-setup)
manages — it came from an import and isn't native TOML yet. The field is derived
from where the definition was loaded, so it's absent for your own tasks and
disappears on its own once you move one into your root config.
...
Tasks that live in the staging file report `"staged": true` in
...
### Graduate an imported task with `promote`

```sh
runwisp promote backup            # move one staged task into runwisp.toml
runwisp promote backup reindex    # move several
runwisp promote --all --reload    # move everything, then reconcile the daemon
```

Staging is a waiting room, not a destination. When you're happy with an
imported job, `runwisp promote` moves its block out of
`runwisp.d/imported.toml` and into your own `runwisp.toml`. After that it's
yours: a re-import leaves it alone, the `staged` marker is gone, and you edit
it like anything else you wrote.

The move is textual, and that's the point — the block's comments, its
formatting, and any unresolved `# TODO` notes the import left behind travel
with it byte-for-byte. Nothing is re-generated, so nothing is lost in
translation. Both files are written as one transaction: if the result wouldn't
load, neither file changes.

| Flag        | What it does                                                         |
| ----------- | -------------------------------------------------------------------- |
| `--all`     | Promote every staged task. Exits 0 when there are none.              |
| `--reload`  | Reload the running daemon afterwards, so the `staged` marker clears. |
...
the `staged` marker the UI and `--json` report. Once the last task is
promoted, the empty `runwisp.d/imported.toml` is removed; the next
`import --write` recreates it.

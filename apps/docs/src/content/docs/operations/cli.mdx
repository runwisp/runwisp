...
sync. If that install used `--system`, pass `--system` here too — there's
no stored record of how the unit was installed, so a mismatch looks at
the wrong unit and falls back to a plain SIGTERM instead of delegating.
...
`--take-over-cron` (Linux, `--system`, root only: stop and mask the system
cron service once RunWisp is confirmed running — see
[`--take-over-cron`](/operations/autostart/#--take-over-cron) for the gates
and rollback behavior; `--allow-skipped-cron-jobs` overrides one of them).
`uninstall` takes `--yes`, `--force`, `--system`, and `--purge` (also delete
the data dir — which makes you type the literal word `delete` to confirm).
`status` also takes `--system`. Whichever of `install`'s `--system` or
its absence you used, repeat it on `status` / `uninstall` — there's no
stored record of which one a given unit was installed with. It's all
covered in depth on the [Autostart](/operations/autostart/) page.

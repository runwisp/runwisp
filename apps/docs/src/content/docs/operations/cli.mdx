...
The summary on stderr gives every job in your crontab or supervisord config one
line, with the command that will actually run and a mark for how it landed:
`✓` mapped cleanly, `~` something changed, `!` needs a fix before this config
loads, `-` skipped because you already have it. That's what `--quiet` trims down
to: it drops the clean rows, not the ones that need you. Writing a config that
won't load and saying nothing about it isn't a mode RunWisp has.

`--dry-run` gives you the same summary plus a list of the files a real run would
create or change — and changes none of them. It's the safe first move on a
machine whose config you care about:

```sh
runwisp import cron /etc/crontab --write --dry-run
```

It proves the generated tasks are valid, and it says so when it can already see
that a real run would refuse — a root `runwisp.toml` that doesn't load today, for
instance. The one thing it can't prove is that the imported tasks merge cleanly
with the ones you already have, since that means writing both files and loading
the result. The output says as much rather than leaving you to assume; the real
run does check it, and rolls both files back if it fails.

`--write` doesn't append to your `runwisp.toml`. It installs the import in a
**two-tier layout**: the generated tasks land in a machine-owned
`runwisp.d/imported.toml` (RunWisp's drop-in directory, named after cron's own
`/etc/cron.d`), and your root `runwisp.toml` is created — or has its
[`[daemon].include`](/configuration/daemon/) wired — so the daemon loads them.
Your hand-authored config stays yours; the importer only ever owns the staging
file. Both files are written together and rolled back as one, so a name that
clashes with a task you already have leaves nothing behind but an error.

If your root config already sets a custom `include` list, `import --write`
won't second-guess it — it tells you the glob to add and writes nothing.

Tasks that live in the staging file report `"source": "staged"` in
`list`/`status --json` (and the API): imported, not yet native.

An import **copies** definitions — it never touches the thing you imported
_from_. Your crontab is still installed, supervisord is still supervising, so
starting the daemon on a fresh import means every job has two schedulers firing
it. Nothing errors, which is what makes it worth saying out loud, so every
import summary ends with the `!` line that says so:
...
Both migration recipes have the commands.
...
a per-task `user`. The
[Converting crontabs](/replacing-cron/converting-crontabs/) and
...
its block ends up in your own `runwisp.toml` and it's yours — a re-import
leaves it alone, the provenance marker is gone, and you edit it like anything
else you wrote.

The definition is copied as text, and that's the point: the block's comments,
its formatting, and any unresolved `# TODO` notes travel with it
byte-for-byte. Nothing is re-generated, so nothing is lost in translation.
Everything is written as one transaction — if the result wouldn't load, no
file changes.

The two provenances differ in one way. A staged block is **moved**: it leaves
`runwisp.d/imported.toml`, and once that file is empty it's removed (the next
`import --write` recreates it). A cron-sourced block is **copied**, because
the crontab _is_ the definition and RunWisp never writes to it. On the next
load your TOML and the crontab say the same thing, RunWisp keeps yours and
skips the cron copy, so you can delete the cron line whenever you like — or
leave it there forever.

| Flag        | What it does                                                           |
| ----------- | ---------------------------------------------------------------------- |
| `--all`     | Promote everything promotable. Exits 0 when there's nothing.           |
...
Promoting doesn't change what the daemon runs — only which file defines it.
So `runwisp reload` after a promote reports _no task changes_, and a promoted
service keeps running without a restart. The reload is only there to refresh
the provenance the UI and `--json` report.
...
stdout), `--dry-run`, `--force` (overwrite a hand-edited unit),
`--binary <path>` (override the baked-in binary path), and
`--take-over-cron`, which answers the "stop and mask the system cron
service?" question the install otherwise asks on a terminal — see
[Take over from cron](/replacing-cron/take-over-from-cron/) for the
refusals and rollback behavior; `--allow-skipped-cron-jobs` overrides one
of them.
...
The front door for the last step of a cron migration. While cron is alive,
RunWisp [holds](/replacing-cron/held-jobs/) the jobs it read from
cron's own files rather than firing them alongside cron; `takeover` hands
them over. It's `service install --take-over-cron` plus a reload of the
running daemon, so held jobs go live immediately instead of waiting for you
to reload by hand.

Takes `--dry-run`, `--force`, `--yes` (`-y`),
`--allow-skipped-cron-jobs`, and `--binary <path>`. Needs root and
systemd; see [Take over from cron](/replacing-cron/take-over-from-cron/)
for the refusals, the ordering, and how to undo it.

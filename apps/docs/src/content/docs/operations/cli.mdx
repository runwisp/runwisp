...
A staged block is **moved** out of the staging file. A cron-sourced one is a real
move too: the block lands in `runwisp.toml` and the exact crontab line it came
from is deleted, in the same transaction — never left behind for a still-live
cron daemon to fire a second time. Promote refuses, writing nothing, if that
line has changed or gone missing since the config was loaded.

What a following `runwisp reload` reports depends on where the task came from.
Promoting a **staged** task — or a cron task you'd already taken over, so RunWisp
was scheduling it anyway — changes nothing about what runs: the reload reports
_no task changes_ and only clears the provenance marker. Promoting a job **while
its cron daemon is still live** is different. RunWisp had been standing down for
that job; with the crontab line gone it's no longer held, so the reload reports
it as a changed task and RunWisp takes over its schedule.

...
line has changed or gone missing since the config was loaded. Promoting doesn't
change what the daemon runs — only which file defines it — so `runwisp reload`
afterwards honestly reports _no task changes_.

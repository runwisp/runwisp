...
You hit the **5 attempts per IP per 5 minutes** rate limit. Wait it out
(the window slides automatically) or restart the daemon to clear the
in-memory limiter. See [Auth: rate limiting][rate].
...
Your JWT expired (24-hour lifetime) **or** you changed
...
The supervisor restarts crashed replicas with [exponential backoff][backoff]
...
[routes]: /notifications/routes/#delivery-failures-the-cycle-break

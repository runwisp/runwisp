...
promote removes the crontab line, so on the next load the job is native and no
longer held. That _is_ a change: the reload reports it as a changed task and
RunWisp takes over scheduling it. It's the whole point of promoting before you
retire cron, and it's why the crontab line has to go in the same step.

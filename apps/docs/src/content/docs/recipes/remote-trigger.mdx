...
<Tabs syncKey="surface">
    <TabItem label="Remote CLI" icon="seti:shell">
        ...
        this), then point [`run`](/configuration/tasks/#run) at the remote daemon:
        ...
        **Parameters?** `runwisp run` doesn't take per-run parameter
        values — a CLI trigger runs the task with its declared
        [`params`](/configuration/tasks/#params) defaults. To supply values,
    ...
    <TabItem label="REST API" icon="cloud-download">
        ...
        their values with a [`params`](/configuration/tasks/#params) object in the request body — the
        ...
        The keys are the parameter keys you declared — the [`env`](/configuration/tasks/#env)/`arg`
        ...
        while :; do
          RUN=$(curl -sSf "$BASE/api/runs/$RUN_ID" \
            -H "Authorization: Bearer $TOKEN")
        ...
        SSE stream at
        `GET /api/runs/$RUN_ID/log/stream`, which emits

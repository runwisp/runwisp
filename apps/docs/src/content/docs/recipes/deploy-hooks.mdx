...
Login is an SRP exchange: the daemon never sees your raw password,
and you don't have to handle it on the shell. The `runwisp auth-token`
subcommand performs the handshake and prints a JWT you can pass to
`curl`.
...
# 1. Obtain a session token via SRP. RUNWISP_REMOTE_PASSWORD never
#    leaves this shell; only proofs derived from it cross the wire.
TOKEN=$(runwisp auth-token --remote "$BASE" --password env)
...
If you don't have the binary on your runner, pipe the password from
stdin and call the daemon directly with two POSTs. Most CI systems do
have the binary available (it's a single static file), so the
subcommand approach is usually simpler.

Alternate password input:

```bash
TOKEN=$(printf '%s\n' "$RUNWISP_REMOTE_PASSWORD" | runwisp auth-token --remote "$BASE" --password -)
```
...
- [`[tasks.*]` reference](/configuration/tasks/) — every key on
  this task: `on_overlap`, `notify_on_failure`, `keep_runs`, etc.
- [Slack provider](/notifications/providers/slack/) — wiring the
  `slack-ops` and `slack-deploys` notifiers.
- [Operations: auth](/operations/auth/) — the CHAP flow your CI
  script is doing.

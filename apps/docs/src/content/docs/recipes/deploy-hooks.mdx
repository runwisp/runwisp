...
title: Deploy hooks
description: Manual-only tasks triggered from CI via the REST API — schema migrations, blue/green flips, cache warmups — with kill-on-overlap so a stale deploy never lingers.
---
...
[`cron`](/configuration/tasks/#cron) on it — instead, your CI pokes it through RunWisp's REST API
...
on_overlap  = "kill"   # a fresh deploy preempts an in-flight one
keep_for    = "180d"        # six months of deploy history
notify = ["slack-ops"]      # failures page ops
...
Leave [`cron`](/configuration/tasks/#cron) off and the task becomes **manual-only** — nothing
...
<Tabs syncKey="surface">
    ...
    <TabItem label="REST API" icon="cloud-download">
        ...
        the full CHAP handshake plus trigger script. On the box itself, [`runwisp run
        deploy-app`](/getting-started/quick-start/#3-run-the-example-task) is the same call wrapped
        in a CLI; from another machine, [`runwisp run deploy-app --url …`](/recipes/remote-trigger/)
        does it remotely.
    </TabItem>
</Tabs>
...
[`"kill"`](/concepts/concurrency/#kill) is
exactly right for this scenario, and almost nothing else.

### A success route to the team channel

[`notify`](/configuration/tasks/#notify) covers failures. Deploys are the one place a _success_ ping
earns its keep — "v1.2.3 deployed at 14:32" is what a team channel wants
— so add a [`[[route]]`](/notifications/routes/) for it:
...
Everywhere else, success pings are noise — which is why they're a
deliberate route, not on by default. The [notifications model][notif]
page goes into why.
...
[CHAP login](/operations/auth/#network-clients-web-ui-remote-rest)
...
Notice that [`on_overlap`](/configuration/tasks/#on_overlap) is `"skip"` here, not `"kill"` —

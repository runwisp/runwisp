...
`validate`, `reload`, [`run`](/configuration/tasks/#run), [`import`](/configuration/compose/#import), and so on. This page is the full
...
drives RunWisp headless. Every document carries a `schemaVersion` so you can
trust its shape across releases. Within one schema version the shape only grows:
fields can be added, but removing or repurposing one requires a new
`schemaVersion`. A failure still exits non-zero **and** prints JSON describing
what went wrong, so your script never has to grep stderr to learn something
broke.
...
report their [`instances`](/configuration/services/#instances) and omit `maxConcurrent` — a service's copy count is
[`instances`](/configuration/services/#instances), not an overlap cap, so there's no value to report:
...
`"sourceFile"` names the exact file. Both are derived from where the definition
...
```sh
runwisp run <task>              # run a task, stream its output, exit with its code
runwisp run <task> --daemon     # require a running daemon (fail fast if none)
runwisp run <task> --standalone # require in-process (refuse if a daemon owns the dir)
runwisp run <task> --json       # print the run outcome as JSON (log lines to stderr)
runwisp run <task> --param key=value --param other=value  # supply parameter values
```

`runwisp run` runs one task and streams its stdout/stderr straight to
...
If the task declares [parameters](/concepts/parameters/), repeat `--param
key=value` for each one you want to set; a parameter you don't mention
falls back to its declared default. `--param` is the CLI's version of the
form the Web UI and TUI show you at trigger time.
...
[`run`](/configuration/tasks/#run) still exits non-zero. So a caller parsing stdout always gets JSON, never
silence:
...
runwisp restart <name>   # restart one service (starts it if it's stopped)
runwisp stop <name>      # stop one service, daemon keeps running
```
...
when you want a fresh boot to re-fire [`run_on_start`](/configuration/tasks/#run_on_start) and missed-run
...
Give either one a service name and it acts on that single service instead of
the whole daemon: `runwisp stop web` stops `web` and leaves everything else
running, and `runwisp restart web` restarts it — or **starts** it, if it was
stopped (a service that booted `autostart = false`, or one a `reload` left
stopped). The daemon isn't touched, so this never delegates to systemd. It's
the same start/stop you get from the dashboard, the TUI, and the REST API,
...
runwisp import systemd [UNIT...]         # convert systemd .service units to runwisp.toml
```

Each reads from a file or from stdin, prints the generated TOML to stdout
...
[Converting crontabs](/coming-from/crontabs/#retiring-the-crontab)
has the commands, along with the two-tier layout, the user column in system
crontabs, and the staging-to-promoted workflow;
[Migrating from supervisord](/coming-from/supervisord/) and
[Migrating from systemd](/coming-from/systemd/) cover the other two sources.

`import systemd` takes `.service` unit files: a unit with `Restart=` becomes a
service, a `Type=oneshot` unit becomes a task, and the service name comes from
the filename. There's no `takeover systemd` — disabling your own service units
isn't something to automate — so run `sudo systemctl disable --now <unit>`
yourself before starting the daemon. The [From systemd](/coming-from/systemd/)
page has the full field mapping and what needs a human.
...
| `--reload`  | Reload the running daemon afterwards so it picks the move up.                               |
...
[Take over from cron](/coming-from/cron/).
...
While cron is alive, RunWisp [holds](/coming-from/cron/) the jobs it
...
[Take over from cron](/coming-from/cron/) for the
...
[`RUNWISP_AUTH=off`](/operations/auth/#running-without-a-password) there's
...
line on a scaffolded config points to. For pointing an AI coding agent at
RunWisp, see the [Driving with an AI agent](/reference/agents/) page — the short
version is `https://docs.runwisp.com/llms.txt`, which `runwisp --help` also
prints when its output isn't a terminal.

## Station and demo

```bash
runwisp station   # connect to the optional control plane instead of scheduling locally
runwisp demo    # boot a throwaway, fully-populated instance to explore
```

`runwisp station` starts the daemon connected to the optional control plane;
it needs `RUNWISP_STATION_TOKEN` (via env, `.env`, or `--token`) and takes
...
directory that's deleted when the daemon stops. It takes `--station` (and
...
| `RUNWISP_STATION_TOKEN`   | Token for `runwisp station`. Ignored in standalone mode.                                                                                                                                                                     |
| `RUNWISP_STATION_URL`     | Control-plane URL for `runwisp station` (overrides the built-in default).                                                                                                                                                    |

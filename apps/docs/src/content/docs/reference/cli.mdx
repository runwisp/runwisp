...
`validate`, `reload`, `run`, `import`, and so on. This page is the full
...
report their `instances` and omit `maxConcurrent` — a service's copy count is
...
`run` still exits non-zero. So a caller parsing stdout always gets JSON, never
silence:

...
- **Fall-back** (clocks go back). The wall-clock minute that repeats —
  ...
  `(local-date, hour, minute)`; the suppressed firing is still
  recorded in history.
...
the task. On the very first boot, the anchor is set the first time
RunWisp sees the task — so a fresh install doesn't queue up a big
batch of "catch-up" runs for ticks before the daemon existed.

### Capping the backfill: `max_catch_up_runs`

`catch_up = "all"` on a `* * * * *` task and a daemon that was down
...
The cap is bounded by a built-in maximum (`10000`) — values above

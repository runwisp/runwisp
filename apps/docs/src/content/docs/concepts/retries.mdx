...
anything you leave off falls back to a sensible default from
...
or `log_overflow` (which is what [`log_on_full = "kill_task"`](/concepts/logs/#rotation-log_max_size-and-log_on_full) does after
...
- **`on_overlap = "terminate"` plus retries.** When a fresh firing
  terminates the running attempt, that attempt ends as `stopped`, which
  **shuts the retry chain down**. The run the terminate policy starts is
...
- **`max_concurrent > 1`.** Retries don't count toward `max_concurrent`.
  A retry only starts once its predecessor has finished, so there's no
  overlap left to evaluate.
...
A service supervisor will restart a replica forever — with bounded
exponential backoff — until you explicitly stop it.

### Backoff reset: `backoff_reset_after`

Once a replica has stayed up for at least `backoff_reset_after` (default
60s), its restart counter resets. That way a service that flapped a
bunch on startup isn't stuck with long restart delays after it finally
settles down. Set it in [`[defaults]`](/configuration/defaults/) to cover every service that doesn't
say otherwise, or per service:

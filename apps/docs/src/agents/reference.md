...
- Inheritance: `[defaults]` → each task/service → per-key override. [`env`](/configuration/tasks/#env) merges (task wins); `[compose.<alias>.<svc>]` overrides per imported service.
...
failures:            []str        — outcomes classified as a failure (stats/UI/notify, and gates retry/restart); tokens are reason names (failed,timeout,crashed,log_overflow,start_failed,missed,stopped,daemon_stopped,skipped,queue_full,dst_skipped; not succeeded), exit codes ("42","1-23"; 1..255), or output patterns ("output:<RE2>", matched per stdout/stderr line after secret redaction; a match turns an exit-0 run into reason failed and writes a system log line). Default [failed,timeout,crashed,log_overflow,start_failed,missed]. is_failure = reason∈tokens OR (reason==failed AND (exit∈ranges OR output matched)). Bare list replaces; all-+/- tokens ("-missed","+stopped") adjust the inherited set (task adjusts [defaults], [defaults] adjusts built-in); mixing bare & +/- rejected ("-output:X" drops only an identical pattern). exit 0 = success unless an output pattern matched. retry_attempts/restart=on_failure fire only when reason∈{failed,timeout,crashed,log_overflow,start_failed} AND is_failure — narrowing failures narrows retry/restart; promoting stopped/missed never makes them retry
...
Per-service override `[compose.<alias>.<svc>]` accepts: [`group`](/configuration/tasks/#group), [`description`](/configuration/tasks/#description), [`timeout`](/configuration/tasks/#timeout), [`graceful_stop`](/configuration/tasks/#graceful_stop), [`stop_signal`](/configuration/tasks/#stop_signal), `restart`, [`instances`](/configuration/services/#instances), [`restart_delay`](/configuration/services/#restart_delay), [`restart_backoff`](/configuration/services/#restart_backoff), [`healthy_after`](/configuration/services/#healthy_after), [`restart_attempts`](/configuration/services/#restart_attempts), [`priority`](/configuration/services/#priority), [`autostart`](/configuration/services/#autostart), [`failures`](/configuration/tasks/#failures), [`log_max_size`](/configuration/tasks/#log_max_size), [`log_on_full`](/configuration/tasks/#log_on_full), [`keep_runs`](/configuration/tasks/#keep_runs), [`keep_for`](/configuration/tasks/#keep_for), [`manual_trigger`](/configuration/tasks/#manual_trigger), [`env`](/configuration/tasks/#env), [`env_file`](/configuration/tasks/#env_file), [`secrets`](/configuration/tasks/#secrets), [`secrets_file`](/configuration/tasks/#secrets_file), [`notify`](/configuration/tasks/#notify). Not allowed: [`run`](/configuration/tasks/#run)/[`compose_file`](/configuration/tasks/#compose_file)/[`compose_service`](/configuration/tasks/#compose_service) (the parent block owns the backend), [`on_overlap`](/configuration/tasks/#on_overlap) (task-only concept), and the host-process keys [`shell`](/configuration/tasks/#shell)/[`umask`](/configuration/tasks/#umask)/[`env_base`](/configuration/tasks/#env_base)/[`user`](/configuration/tasks/#user). `import="stack"` forbids overrides and the [`services`](/configuration/compose/#services) filter. Per-service [`notify`](/configuration/tasks/#notify) desugars into notify routes keyed by the generated task name, exactly like `[services.*]`. The reserved sub-table `[compose.<alias>.defaults]` accepts the same keys and applies them to every imported service before the per-service override wins (precedence: compose-import default → `defaults` → `<svc>`); its [`notify`](/configuration/tasks/#notify) adds routes to all services. A compose service literally named `defaults` is rejected (rename hint); `import="stack"` forbids `defaults` too.

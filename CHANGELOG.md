...
- **`runwisp import … --write` installs a two-tier layout.** Imported jobs land in a machine-owned `runwisp.d/imported.toml` and your root `runwisp.toml` is created (or has its `include` wired) to load them, atomically — so your hand-authored config stays yours. `-o` and stdout are unchanged. See [Migrating from cron](https://docs.runwisp.com/recipes/migrating-from-cron/).
- **`runwisp import` accounts for every job in the source file** — a row each, marked mapped cleanly, changed, needs-a-fix, or already-yours, plus `--dry-run` to preview the whole thing and a closing warning that your crontab still runs the jobs it just copied. See [Migrating from cron](https://docs.runwisp.com/recipes/migrating-from-cron/#the-one-liner).
...
- **`sudo runwisp service install` offers to retire the system cron service.** When cron is running and RunWisp is reading its crontabs, the install asks whether to stop and mask `cron.service`/`crond.service`/`cronie.service` (Linux only) — the alternative being every job firing twice. It masks only once RunWisp is confirmed running, rolls the mask back if RunWisp fails to start, and doesn't ask at all if any cron source isn't loading cleanly or would run a task as a user this daemon can't become. `--take-over-cron` answers the question for scripted installs; `--yes` deliberately doesn't. `service uninstall` and `service status` know about the take-over too. See [Retiring cron](https://docs.runwisp.com/operations/autostart/#retiring-cron).

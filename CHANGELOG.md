...
- **Imported-but-not-yet-native tasks are marked `staged`** in `runwisp list`/`status --json` (and the API), so it's clear which tasks came from an import versus your own TOML. See [CLI](https://docs.runwisp.com/operations/cli/#machine-readable-output-json).

...
- **`runwisp service install --system --take-over-cron` retires the system cron service for you.** It stops and masks `cron.service`/`crond.service`/`cronie.service` (Linux only) once RunWisp is confirmed running, refuses up front if any cron source isn't loading cleanly or would run a task as a user this daemon can't become, and rolls the mask back if RunWisp fails to start. `service uninstall --system` and `service status` know about the take-over too. See [`--take-over-cron`](https://docs.runwisp.com/operations/autostart/#--take-over-cron).
...
- **`service install --system` refuses a config path that isn't root-owned and locked down.** It now checks the resolved `runwisp.toml` the same way a cron-sourced file is checked before baking its path into a root-run unit. See [Trust model](SECURITY.md#trust-model).

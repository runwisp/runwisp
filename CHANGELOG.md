...
- **Schema and CLI tidy-up before 1.0 (BREAKING).** A coordinated rename pass that removes accumulated naming inconsistencies. None of these change what the daemon can do — only how you spell it. Renames live in one place so the migration cost is paid once.
  ...
  - Notification model: `[notify] disable_inapp = true` is replaced by `[notify] default_notifiers = []`. The same setting now also lets you redirect the zero-config catch-all (e.g. `default_notifiers = ["slack-ops"]` to make every failure page Slack instead of, or alongside, the in-app bell). Per-task `notify_on_failure` / `notify_on_success` continue to work; the implicit `inapp` is replaced by the contents of `default_notifiers`, deduped against the explicit list.
  - **Retention and log-size keywords (BREAKING).** `keep_runs`, `keep_for`, and `log_max_size` now share a single shape: a positive value caps, the `"unlimited"` keyword opts out of any inherited cap, and omitting the key inherits from `[defaults]`. The numeric sentinels are gone — `keep_runs = -1`, `keep_runs = 0`, `keep_for = ""`, `log_max_size = 0`, and any other zero/negative form are **rejected at config load**. Migration: replace `keep_runs = -1` with `keep_runs = "unlimited"`, drop any `keep_runs = 0` (the field already defaults to "no cap"), and replace `log_max_size = 0` with `log_max_size = "unlimited"` if you actually wanted no cap. `log_max_size = 0` previously did **not** mean unbounded — the loader silently fell through to the built-in 100 MB; the new strict rejection makes that footgun impossible.

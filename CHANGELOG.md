...
- **`runwisp import` accounts for every job in the source file** — a row each, marked mapped cleanly, changed, needs-a-fix, or already-yours, plus `--dry-run` to preview the whole thing and a closing warning that your crontab still runs the jobs it just copied. See [Migrating from cron](https://docs.runwisp.com/replacing-cron/converting-crontabs/#the-one-liner).
...
- **`sudo runwisp takeover` retires cron in one command.** It installs RunWisp as a system service, stops and masks `cron.service`, and reloads the running daemon, so the jobs cron was holding become RunWisp's — the reload being the part `service install --take-over-cron` can't do for a daemon that's already up. Same checks, same ordering, and the same rollback if RunWisp fails to start. See [Retiring cron](https://docs.runwisp.com/replacing-cron/take-over-from-cron/).
...
- **`sudo runwisp service install` offers to retire the system cron service.** When cron is running and RunWisp is reading its crontabs, the install asks whether to stop and mask `cron.service`/`crond.service`/`cronie.service` (Linux only) — the alternative being every job firing twice. Nothing is masked until the unit is written, and the mask is rolled back (cron unmasked and restarted) if RunWisp then fails to start, so the box is never left without a scheduler. It doesn't ask at all if any cron source isn't loading cleanly or would run a task as a user this daemon can't become. `--take-over-cron` answers the question for scripted installs; `--yes` deliberately doesn't. `service uninstall` and `service status` know about the take-over too. See [Retiring cron](https://docs.runwisp.com/replacing-cron/take-over-from-cron/).
...
- **`runwisp service …` no longer ignores the root config/data defaults.** The service commands skip the root command's setup hook so they never create a data dir the operator hasn't consented to — but that also skipped path resolution, leaving `--config`/`--data` empty when unset. See [Autostart](https://docs.runwisp.com/operations/autostart/#resolving-paths).

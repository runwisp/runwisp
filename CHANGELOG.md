...
- **Compose per-service overrides (`[compose.<alias>.override.<svc>]`) can now set `failures`**, matching what `[services.*]` already allows.
...
- **`[compose.<alias>]` per-service overrides now nest under `[compose.<alias>.override.<svc>]`** (block defaults under `.override.defaults`) instead of sitting flat alongside the reserved keys.

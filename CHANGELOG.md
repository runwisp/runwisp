...
- **`runwisp import cron` reproduces cron's working directory.** Cron runs a job in the home directory of the user running it; RunWisp defaults to the daemon's, so a `run` script using relative paths silently wrote somewhere else. A per-user crontab now imports with `working_dir = "~"`. A system crontab's user column can't be resolved that way — `working_dir` is evaluated once at config load, against the daemon's home — so it's left unset and the summary says so once for the file. See [Migrating from cron](https://docs.runwisp.com/recipes/migrating-from-cron/#how-the-pieces-map).

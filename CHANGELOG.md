...
- **`runwisp service install` wires up systemd or launchd autostart.** One command writes a managed user unit (systemd on Linux/WSL) or LaunchAgent (launchd on macOS), enables it, and turns on linger so the daemon survives logout. Re-running is a noop on an unchanged host, prompts before overwriting drift, and refuses to clobber a hand-edited unit without `--force`. The unit is named `runwisp-<fingerprint>.service` (LaunchAgent label: `com.runwisp.daemon.<fingerprint>`), so multiple RunWisp instances on the same machine install side-by-side without collisions. The companion commands are symmetric: `service uninstall` stops and removes everything (data dir preserved unless `--purge`, which requires typing the literal word `delete`); `service status` answers "will the daemon come up after a reboot?" in one screen with exit codes 0 (healthy) / 1 (degraded) / 2 (not installed). `--print` emits the rendered unit to stdout for Ansible / Nix declarative workflows. WSL gets the systemd path plus a printed `Register-ScheduledTask` PowerShell recipe for the Windows side. No root, no surprise sudo — `loginctl enable-linger` is the only escalation and it's announced before it runs.
...
- **Web UI design refresh.** Every panel — overview cards, run history, task detail, notifications popover — now snaps to the same border radius, border tone, and shadow weight; subtle hue drift between the sidebar and the page body is gone. A new connection pip lives in the topbar so daemon reachability is visible from every page, not just the sidebar. Empty states across runs, tasks, and notifications share one shape and tone — including the run detail panel when no run is selected. The runs list is now **virtualized and server-paginated** — it stays smooth on tasks with tens of thousands of runs, scrolls infinitely as you reach the bottom, and routes search, status filter, and sort order through the server instead of clipping at a 200-row client cap.

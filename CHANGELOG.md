...
- **Prometheus-compatible `/metrics` endpoint.** The daemon now serves OpenMetrics 1.0 text at `/metrics` so you can plug RunWisp into Prometheus, Grafana Agent, OpenTelemetry Collector, or any other scraper without parsing the JSON API. The surface includes run counters, the timestamp of the last failure, per-task active-run gauges, daemon CPU / memory / uptime, and a `runwisp_build_info` label. The endpoint is public (parallel to `/health`) and always on — bind to loopback or firewall the port if the daemon is internet-facing. See [Operations / Metrics](https://docs.runwisp.com/operations/metrics/) for the full list of metrics and a sample scrape config.

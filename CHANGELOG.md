...
- **Cloud dispatch opt-in covers container/compose.** Peer-dispatched ad-hoc `container` and `compose` runs now require `allow_cloud_dispatch` like `shell`; previously they were accepted with the opt-in off. See [Daemon](https://docs.runwisp.com/configuration/daemon/).

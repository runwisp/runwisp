...
- **Imported-but-not-yet-native tasks are marked `staged`** in `runwisp list`/`status --json` (and the API), so it's clear which tasks came from an import versus your own TOML. The dashboard shows a `staged` badge and the TUI inspector names the staging file, each pointing at the `runwisp promote` command that graduates the task. See [CLI](https://docs.runwisp.com/operations/cli/#machine-readable-output-json).
...
- **`runwisp promote [TASK...] | --all` graduates an imported task into your own `runwisp.toml`.** The task's block moves out of `runwisp.d/imported.toml` verbatim — comments, formatting, and any unresolved `# TODO` notes included — and both files are written as one transaction, so if the result wouldn't load neither file changes. `--dry-run` shows what would move; `--reload` reconciles the daemon afterwards. `runwisp list` now points out when tasks are still staged. See [CLI](https://docs.runwisp.com/operations/cli/#graduate-an-imported-task-with-promote).

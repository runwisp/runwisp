...
- **`[daemon] include_cron` reads your real crontabs as live task definitions.** Point it at `/etc/crontab`, `/etc/cron.d/*`, or `/var/spool/cron/crontabs/*` and every job becomes a RunWisp task — captured output, history, notifications — with nothing converted or rewritten; `crontab -e` plus `runwisp reload` picks up edits. Per-user crontabs run as the account that owns them (root daemon required) and globs read only the files cron itself would schedule, so a `.dpkg-old` or `.disabled` leftover stays out of your schedule. A job RunWisp can't reproduce is skipped and reported by `file:line` while the rest of the file keeps running, as is a `MAILTO=` nothing is honouring. Cron's firing semantics are kept: a missed tick is recorded but never re-fired. See [`include_cron`](https://docs.runwisp.com/configuration/daemon/#include_cron).
- **`runwisp promote [TASK...] | --all` moves a derived task's definition into your own `runwisp.toml`**, verbatim — an imported block moves out of the staging file, a crontab-sourced one is copied (the crontab is never written). Atomic across both files, with `--dry-run` and `--reload`. See [CLI](https://docs.runwisp.com/operations/cli/#take-ownership-of-a-task-with-promote).
- **`runwisp import … --write` installs a two-tier layout.** Imported jobs land in a machine-owned `runwisp.d/imported.toml` and your root `runwisp.toml` is created (or has its `include` wired) to load them, atomically — so your hand-authored config stays yours. `-o` and stdout are unchanged. See [Converting crontabs](https://docs.runwisp.com/replacing-cron/converting-crontabs/).
- **`runwisp import` accounts for every job in the source file** — a row each, marked mapped cleanly, changed, needs-a-fix, or already-yours, plus `--dry-run` to preview the whole thing and a closing warning that your crontab still runs the jobs it just copied. See [Converting crontabs](https://docs.runwisp.com/replacing-cron/converting-crontabs/#the-one-liner).
- **Tasks carry their provenance** as `source`/`source_file` in `runwisp list`, `status --json` and the API, badged in the dashboard and named in the TUI inspector — so it's clear which tasks came from an import or a crontab rather than your own TOML. See [CLI](https://docs.runwisp.com/operations/cli/#machine-readable-output-json).
- **`runwisp reload`, `runwisp status`, `/api/info` and the TUI header report the live config's warnings**, not just boot and `runwisp validate` — a `crontab -e` that broke a job no longer reloads silently. See [Reload](https://docs.runwisp.com/operations/reload/).
...
- **First run now finds a readable crontab on its own and offers to read it live**, the same way it already offers to import an adjacent `docker-compose.yml` — both can fire together, sharing one prompt. A crontab this account can't safely read (wrong permissions, an unresolvable owner) is reported instead of being scaffolded into a config that wouldn't load. See [Take over from cron](https://docs.runwisp.com/replacing-cron/take-over-from-cron/).
- **`scripts/install.sh` — the script behind `curl -fsSL https://get.runwisp.com | sh` — now exists in this repo.** ~130 lines of POSIX `sh`: detect OS/arch, download the matching release tarball, verify its SHA-256 against the release's `checksums-sha256.txt`, and install to `/usr/local/bin` (root) or `~/.local/bin`. It makes no decisions about your cron jobs, `runwisp.toml`, or init system — that's the binary's job. See [Quick start](https://docs.runwisp.com/getting-started/quick-start/).
- **`sudo runwisp takeover` retires cron in one command, from nothing.** On a box with cron jobs and no `runwisp.toml`, it writes the config that reads your crontabs, installs RunWisp as a system service, stops and masks `cron.service`, and reloads — so the jobs cron was holding become RunWisp's. It shows the whole plan and asks once; `--dry-run` prints that plan and stops, `--yes` runs it unattended. An existing config is wired for `include_cron` if it lacks it, and never rewritten if it already has one. Anything RunWisp can't fix itself — no cron on the box, not root, a config that won't load — is reported up front, all of it at once, before anything is written. Cron is unmasked and restarted if RunWisp then fails to start. See [Retiring cron](https://docs.runwisp.com/replacing-cron/take-over-from-cron/).
- **First run on a root Linux box does the whole cron cutover from one prompt.** Where it used to offer only to read your crontabs, `runwisp` now offers to read them, install itself as a system service, and mask cron — all three stated up front, one `[Y/n]`. Where the take-over isn't possible (non-root, no systemd, macOS) it says so rather than implying cron has been dealt with. See [Retiring cron](https://docs.runwisp.com/replacing-cron/take-over-from-cron/#the-first-run-prompt).
- **`sudo runwisp service install` points at `takeover` when cron is still running.** The install itself only wires up the unit — it never touches cron — but it now says so, naming the live cron unit, how many jobs it owns, and the one command that hands them over. `service uninstall` still unmasks and restarts a cron it can prove RunWisp masked, and `service status` still reports whether it came back. See [Retiring cron](https://docs.runwisp.com/replacing-cron/take-over-from-cron/).
...
- **A job read from a live crontab is now held instead of scheduled.** While a system cron daemon is still alive and still owns the file, RunWisp loads and lists the job, shows its schedule, and does not fire it — cron is already firing it, and two schedulers running one job is the failure `include_cron` exists to avoid. Held jobs are called out at boot, in `runwisp status` and `runwisp validate`, in the TUI and in the dashboard, and carry `held_by` in the API; manual triggers (`runwisp exec`, the API, TUI `r`) still run them, so you can verify a job under RunWisp before committing. The hold window records no runs and no missed ticks, so handing the jobs over doesn't arrive with a backlog of alerts for ticks cron ran fine. The hold ends on its own: the daemon re-checks whether cron is alive about once a minute, so retiring cron by any means hands the jobs over within a minute with no reload needed — and if cron comes back, RunWisp stands down again. A crontab-format file outside cron's own paths is deliberately not held. See [Held jobs](https://docs.runwisp.com/replacing-cron/held-jobs/).
- **`runwisp service install` now installs the system-wide service by default; `--local` gets you a per-user unit.** `sudo runwisp service install` writes `/etc/systemd/system/runwisp.service` — one daemon per host, running as root, no fingerprint in the name. The old per-user unit (`~/.config/systemd/user/runwisp-<fingerprint>.service`, or a LaunchAgent on macOS) is now behind `--local`, and `--system` is gone. Without root and without `--local`, install refuses and names both ways forward rather than quietly installing a unit that dies with your login session. macOS has no system-wide install yet, so `--local` is required there. `--data`/`--config` override paths but never change the scope. See [Autostart](https://docs.runwisp.com/operations/autostart/).
...
- **`runwisp import cron` emits crontab-wide settings per task instead of daemon-wide.** A crontab's `SHELL`, `CRON_TZ`/`TZ`, and top-of-file `VAR=val` lines become each task's own `shell`, `timezone`, and `env` rather than `[defaults]` and `[scheduler] timezone`, so importing leaves your global settings alone and two crontabs can't overwrite each other. See [How cron maps to TOML](https://docs.runwisp.com/replacing-cron/cron-mapping/#the-mapping-table).
- **`runwisp import cron` keeps cron's firing semantics.** Imported jobs get `catch_up = "skip"`, because cron has no missed-tick concept and a daemon started at 15:00 shouldn't re-run last night's 02:00 backup — the gap is still recorded as a missed run, which is more than cron gave you. `on_overlap = "queue"` is written out alongside it so the difference from cron's unbounded overlap is visible and one line to change. See [How cron maps to TOML](https://docs.runwisp.com/replacing-cron/cron-mapping/#the-mapping-table).
- **`runwisp import --quiet` silences the inventory, not the alarm** — a clean import prints nothing, but jobs that need a fix are still listed. See [CLI](https://docs.runwisp.com/operations/cli/#migrate-an-existing-setup).
- **New "runbook" visual design across the web UI and docs**: a unified terminal-operator look shared with the website — monospace headings and chrome, hairline borders, small radii, teal accent, and a flat borders-only dark mode. Every `@runwisp/ui` component and both `apps/ui` and `apps/docs` were restyled; no configuration or behaviour changed.
- **The web UI login screen was redesigned**: a purpose-built sign-in card with the RunWisp brand lockup, a lock-prefixed password field, and the password-source hint tucked behind a "Where's my password?" popover.
- **The web UI no longer animates hover, focus, or state changes.** Buttons, links, table rows, menus, popovers, and inline validation messages now respond instantly instead of fading over ~200ms. Motion is kept only where the movement carries meaning: spinners and skeletons, the live-run pulse, the mobile sidebar slide, progress bars, and accordion height.
- **Geist Mono is the interface monospace**, and it now ships embedded in the binary — the web UI previously named a font it didn't bundle, so monospace text fell back to whatever the operator's OS provided. Identifiers (task names, run IDs, cron expressions, durations, exit codes) render monospace everywhere; prose stays in the body sans.

### Fixed

- **`runwisp status` and the TUI header now report the live task set, not the one from boot.** Both read `/api/info`, which served a list built when the daemon started — so a task a reload added, removed or changed kept being reported as it was at boot, and a job whose cron hold had been released still showed as held by cron. The list is rebuilt per request now. See [`runwisp status`](https://docs.runwisp.com/operations/cli/#look-before-you-leap).
- **A live `cronie.service` now holds the jobs it owns, like `cron.service` and `crond.service` already did.** The hold gate checked two unit names while the cron take-over knew about three, so on a box whose cron registers under the third one RunWisp saw no cron daemon, held nothing, and every job in those crontabs fired from both schedulers with nothing on any surface saying so. Both sides read one list now. See [Held jobs](https://docs.runwisp.com/replacing-cron/held-jobs/).
- **`runwisp validate`'s held-jobs block reads as a sentence again.** It spliced the cron daemon's state in as a noun phrase, so every validate against a box with a live cron printed "a system cron daemon is running still owns them". See [Held jobs](https://docs.runwisp.com/replacing-cron/held-jobs/#where-you-see-a-hold).
- **A single unreadable, untrusted, or otherwise unrunnable crontab in `include_cron` no longer takes down the whole config load.** It's reported as a skipped source and every other cron file (and every native task) still loads — under `Restart=on-failure` a rejected load used to mean a five-second restart loop on a box now running nothing at all. Loading still fails outright if every matched crontab fails at once, since that points at a broken `include_cron` pattern rather than one bad file. See [`include_cron`](https://docs.runwisp.com/configuration/daemon/#include_cron).
- **`include_cron` globbing a per-user spool directory no longer drops a crontab for an account with a dot or a trailing `$` in its name.** `john.doe` or a `svc$` service account is a normal login name that crond reads without complaint; the glob filter was applying the stricter `/etc/cron.d` naming rule to spool directories too and skipping them with a message ("crond ignores this name") that wasn't true. See [Globs read what cron would read](https://docs.runwisp.com/configuration/daemon/#globs-read-what-cron-would-read).
- **An `include_cron` directory that exists but can't be listed by this daemon is now reported, instead of silently scheduling nothing.** `/var/spool/cron/crontabs` ships owner- and group-restricted on a real box, and a plain glob can't tell "nothing here" apart from "couldn't look." See [Globs read what cron would read](https://docs.runwisp.com/configuration/daemon/#globs-read-what-cron-would-read).
- **The "cron is still running" boot warning now also fires for a spool-only `include_cron`, and treats a stopped-but-enabled cron service as still a risk.** It used to only compare against system crontab paths (so a per-user-only setup — the easiest one to try first — got no warning at all) and only checked whether a pidfile existed, not whether the process in it was still alive.
...
- **A system crontab line missing its user column is skipped, not scheduled under a made-up user.** `* * * * * echo "tick"` in `/etc/cron.d` was read as the command `"tick"` running as user `echo` — a live task nobody wrote, failing on every firing. The sixth field now has to name an account that exists on the machine, the same test cron applies, and a line that fails it is reported by `file:line` while the rest of the file keeps running. See [How cron maps to TOML](https://docs.runwisp.com/replacing-cron/cron-mapping/#what-needs-a-human).
- **`service status`, `service uninstall`, `stop`, and `restart` find whichever unit is installed.** They used to need to be told, and a mismatch meant looking at the wrong unit: reporting "Nothing to uninstall ✓" while the real service stayed installed and running, or `--purge` deleting the wrong data dir. They now look for the unit, and ask for `--local` only when both a system and a per-user unit exist. See [Autostart](https://docs.runwisp.com/operations/autostart/).
- **A system service install refuses a config path that isn't root-owned and locked down.** It checks the resolved `runwisp.toml` the same way a cron-sourced file is checked before baking its path into a root-run unit. See [Trust model](SECURITY.md#trust-model).
- **`runwisp service install --dry-run` now runs the checks it was only claiming to.** It printed the plan and returned before looking at what holds the port, so a dry run could report a clean install that then stopped before writing anything — including the case where your own hand-started daemon is in the way. The plan still prints; the conflict is added to it, with a non-zero exit. `--print` is unchanged and still writes only the unit. See [Autostart](https://docs.runwisp.com/operations/autostart/#flag-reference).
- **`runwisp service …` no longer ignores the root config/data defaults, or `RUNWISP_PORT`.** The service commands skip the root command's setup hook so they never create a data dir the operator hasn't consented to — but that also skipped path and port resolution, leaving `--config`/`--data` empty when unset and baking the default port into the unit even when the environment asked for another one. See [Autostart](https://docs.runwisp.com/operations/autostart/#resolving-paths).
- **`sudo runwisp takeover` works on a box that's already running RunWisp** — which is most of them. The install refused any occupied port, including one held by its own service, so the reload that hands the held jobs over was unreachable: the take-over stopped before it ever masked cron. A port held by this install's own service is no longer a conflict, and a daemon you started by hand gets told to stop rather than "run on a different port", which would have left two daemons on one database. Installing over a live service now also notes that its settings only take effect on restart. See [Take over from cron](https://docs.runwisp.com/replacing-cron/take-over-from-cron/#what-happens-in-order).
- **Re-running `sudo runwisp takeover` retires a cron that came back.** An operator unmasking cron (or a package upgrade doing it) left `service status` reporting "jobs may be firing twice" with no command to fix it: the unit already matched, so the install printed "Already installed ✓" and returned without looking at cron. It now retires it again — the plan names the stop and the mask as the only steps left, and one confirmation covers them — then checks RunWisp is actually up and rolls cron back if it isn't. See [Undoing it](https://docs.runwisp.com/replacing-cron/take-over-from-cron/#undoing-it).
- **Asking to retire cron on macOS says what to do instead.** `runwisp takeover` answered with the system-service scope error ("re-run with `--local`"), which cannot retire cron either. It now refuses up front and names the manual route: `runwisp import cron --write`, then `crontab -r`. See [macOS](https://docs.runwisp.com/replacing-cron/take-over-from-cron/#macos-not-possible).
- **A day-of-week of `7` means Sunday, as it does in cron.** RunWisp rejected it outright, so Debian's and Ubuntu's own `/etc/crontab` line `47 6 * * 7 root … run-parts /etc/cron.weekly` was skipped on every stock box — taking that box's weekly housekeeping with it. `0` and `7` are both Sunday now, in lists and ranges too. See [How cron maps to TOML](https://docs.runwisp.com/replacing-cron/cron-mapping/#the-mapping-table).
- **`sudo runwisp takeover` asks for `--allow-skipped-cron-jobs` at the same point every time it runs.** A job it can't reproduce inside an otherwise-readable crontab was only noticed once a `runwisp.toml` existed, so the first take-over on a bare box went through without asking and a re-run of the same untouched box then hard-blocked — breaking the "safe to re-run in a provisioning script" promise. See [Take over from cron](https://docs.runwisp.com/replacing-cron/take-over-from-cron/).
- **`runwisp status` no longer reports "config changed" forever after an untouched take-over.** The staleness check re-expanded `include_cron` globs without cron's own naming rules, so a file crond itself skips — `/etc/cron.d/.placeholder`, shipped by the `cron` package on every Debian and Ubuntu box — read as a change on every request, and a restart didn't clear it. See [Reload](https://docs.runwisp.com/operations/reload/).

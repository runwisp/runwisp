...
- **Web UI design refresh.** Every panel — overview cards, run history, task detail, notifications popover — now snaps to the same border radius, border tone, and shadow weight; subtle hue drift between the sidebar and the page body is gone. A new connection pip lives in the topbar so daemon reachability is visible from every page, not just the sidebar. Empty states across runs, tasks, and notifications share one shape and tone — including the run detail panel when no run is selected. List motion is gentler too: rows fade in when they appear, animate when you flip sort order, and the active-row indicator slides smoothly to the new selection instead of teleporting.

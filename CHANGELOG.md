...
- **`treat_missed_as_failure` and `exit_codes` are replaced by one `failures` key** on `[defaults]`, `[tasks.*]`, and `[services.*]`. List the outcomes that count as a failure — end-reason names (`failed`, `timeout`, `crashed`, `missed`, `stopped`, …) and exit codes or inclusive ranges (`"42"`, `"1-23"`) — to drive the failed stat, UI badges, and `notify_on_failure`. It works both ways: drop `missed` to stop paging on a missed tick, add `stopped` to page on a killed run. Exit `0` is always success and any non-zero exit is `failed`; there's no success-code allowlist anymore. See [What counts as a failure](https://docs.runwisp.com/configuration/tasks/#what-counts-as-a-failure).
...
- **`[compose.<alias>.defaults]`**: a reserved sub-table whose policy keys (`restart`, `restart_attempts`, `exit_codes`, `notify_on_*`, …) apply to every imported compose service, with per-service overrides still winning — set a whole worker pool's `restart = "always"` in one line. See [Block defaults](https://docs.runwisp.com/configuration/compose/#block-defaults).
...
- **Several `runwisp.toml` keys were renamed for consistency.** Service `start_retries` → `restart_attempts` (matching `restart_delay`/`restart_backoff`); the missed-run toggle `notify_on_missed` → `treat_missed_as_failure` (a missed scheduled run is a failure that rides `notify_on_failure`'s channels — it was never its own channel list); the notification-route `match.kind` → `match.kinds` (it takes an array); and SMTP `tls = "none"` → `tls = "off"` (matching `[daemon] tls`). The redundant param key `desc` was removed — use `description`. See [Configuration](https://docs.runwisp.com/configuration/).
- **`keep_runs = 0` now means "keep no completed runs" instead of "inherit the default".** Omit the key to inherit from `[defaults]`; set `0` to retain nothing. The accepted range is now `0`–1 000 000. See [Logs & retention](https://docs.runwisp.com/concepts/logs/#retention-keep_runs-and-keep_for).
- **The REST API and SSE event payloads are now camelCase throughout.** Every JSON body field, query parameter, and streamed field uses camelCase (`task_name` → `taskName`, `exit_code` → `exitCode`, …); `runwisp.toml` keys are unchanged and stay snake_case, and `runwisp … --json` output follows the API. Operation IDs were normalized (`getTasks`/`getAllRuns` → `listTasks`/`listRuns`, `restartService`/`stopService` → `restartTask`/`stopTask`, `bulkCancelRuns` → `bulkStopRuns`); the redundant task-scoped `GET /api/tasks/{taskName}/runs/{runId}` was dropped in favour of `GET /api/runs/{runId}`; `POST /api/runs/bulk/cancel` became `POST /api/runs/bulk/stop`; and the run stop/restart endpoints now return `204 No Content`.
- **`RUNWISP_TRUST_PROXY` was renamed `RUNWISP_TRUSTED_PROXIES`** to match the `trusted_proxies` config key. See [Reverse proxies](https://docs.runwisp.com/operations/auth/#reverse-proxies).
...
- **`restart_attempts` on `[services.*]` / `[defaults]`, plus FATAL services.** A service that keeps fast-failing (exits below `healthy_after`) now tolerates `restart_attempts` failed starts in a row before going FATAL — it stops restarting, records a `start_failed` run, and rings the bell instead of flapping forever. See [When a service can't start](https://docs.runwisp.com/configuration/services/#when-a-service-cant-start-fatal).

...
- **`runwisp import … --write` now installs a two-tier layout.** Imported jobs land in a machine-owned `runwisp.d/imported.toml` (RunWisp's drop-in directory, named after cron's own `/etc/cron.d`), and your root `runwisp.toml` is created (or has its `include` wired) to load them — so your hand-authored config stays clean while imports are staged alongside it. Both files are written atomically: a name that clashes with a task you already own rolls the whole write back instead of leaving the daemon with a config it can't load. For `import cron`, a re-import skips a job whose command you already own and renames a genuine clash. Piping to stdout or writing a standalone file with `-o` is unchanged. See [Migrating from cron](https://docs.runwisp.com/recipes/migrating-from-cron/).
- **`[daemon] include_cron` reads your real crontabs as live task definitions.** Point it at `/etc/crontab` or `/etc/cron.d/*` and every job in those files becomes a RunWisp task — captured output, run history, notifications — with nothing converted and nothing rewritten. `crontab -e` plus `runwisp reload` picks up edits, and `runwisp promote` graduates a job into your own TOML when you're ready. A job RunWisp can't reproduce is skipped and reported by `file:line` while the rest of the file keeps running; a crontab anyone but root or the daemon's own user could write is refused outright. See [`include_cron`](https://docs.runwisp.com/configuration/daemon/#include_cron).
- **`runwisp reload`, `runwisp status`, `/api/info` and the TUI header now report the live config's warnings.** Previously they were printed once at boot and by `runwisp validate`, so a `crontab -e` that broke a job — followed by the reload that was supposed to pick it up — said nothing about the job that didn't come back. A skipped job has no runs to make it visible, so these are the only places it can be seen. See [Reload](https://docs.runwisp.com/operations/reload/).
- **Imported-but-not-yet-native tasks are marked `staged`** in `runwisp list`/`status --json` (and the API) via `source`/`source_file`, so it's clear which tasks came from an import or a crontab versus your own TOML. The dashboard badges them and the TUI inspector names the file, each pointing at the `runwisp promote` command that graduates the task. See [CLI](https://docs.runwisp.com/operations/cli/#machine-readable-output-json).
- **`runwisp import` gives every job in the source file its own line in the summary**, marked `✓` mapped cleanly, `~` imported with a change, `!` needs a fix before the config loads, or `-` skipped because you already have it. Each row shows the command that will actually run, wrapped rather than truncated. A job that produced no TOML at all — an `[eventlistener]`, an unreadable crontab line — still gets a row, so nothing leaves the import unaccounted for. See [Migrating from cron](https://docs.runwisp.com/recipes/migrating-from-cron/#the-one-liner).
- **`runwisp import … --dry-run` prints the summary and every file a real run would touch, and writes nothing.** The safe first move on a machine whose config you care about. See [CLI](https://docs.runwisp.com/operations/cli/#migrate-an-existing-setup).
- **`runwisp import` warns that the source scheduler still runs the jobs it copied.** An import doesn't disable your crontab or stop supervisord, so starting the daemon on a fresh import fires every job twice — silently. The summary now ends by saying so, and both migration recipes cover retiring the old scheduler first. See [Migrating from cron](https://docs.runwisp.com/recipes/migrating-from-cron/#nothing-has-retired-cron-yet).
- **`working_dir = "~"` is the home of whoever the task runs as**, not always the daemon's. On a task that sets `user`, `~` now resolves to that account's home, looked up per run alongside the credential itself — so it works for a user who doesn't exist on the machine you ran `runwisp validate` from. This is cron's own rule, and it's what makes a system crontab importable without hand-writing a path per job. See [Tasks](https://docs.runwisp.com/configuration/tasks/#working-directory--shell).
- **`env_base` picks what a task's environment starts from.** The default, `inherit`, is what RunWisp has always done — the daemon's own environment, minus its `RUNWISP_*` internals. `clean` starts from the minimal set cron gives a job (`PATH=/usr/bin:/bin`, `SHELL`, `HOME`, `USER`/`LOGNAME`) and nothing else, so a task's behaviour stops depending on how the daemon happened to be launched. `runwisp import cron` sets it on everything it imports, because that's the environment those jobs were written against. Host shell runs only. See [Tasks](https://docs.runwisp.com/configuration/tasks/#working-directory--shell).
- **A `sendmail` notifier mails through the MTA already on the box.** `type = "sendmail"` pipes a plain-text message to the host's sendmail binary — Postfix, exim, msmtp — the same way crond delivered `MAILTO` output, so there's no relay host, port, or password to configure. `runwisp import cron` now answers a `MAILTO=` line with the exact notifier block to paste, instead of only saying that RunWisp doesn't email job output. See [Email (local MTA)](https://docs.runwisp.com/notifications/providers/sendmail/).
- **`runwisp promote [TASK...] | --all` puts a derived task's definition in your own `runwisp.toml`.** It works on both provenances: an imported task's block moves out of `runwisp.d/imported.toml` verbatim, and a task read live from a crontab is copied from the exact text the daemon decoded to run it (the crontab itself is never written) — comments, formatting, and any unresolved `# TODO` notes included — and both files are written as one transaction, so if the result wouldn't load neither file changes. `--dry-run` shows what would move; `--reload` reconciles the daemon afterwards. `runwisp list` now points out when tasks are still derived rather than yours. See [CLI](https://docs.runwisp.com/operations/cli/#take-ownership-of-a-task-with-promote).
...
- **`runwisp import --quiet` silences the inventory, not the alarm.** A clean import prints nothing, but one with jobs that need a fix still lists those jobs — writing a config that won't load and saying nothing about it is the failure mode `--quiet` is most likely to be scripted into. See [CLI](https://docs.runwisp.com/operations/cli/#migrate-an-existing-setup).
- **`runwisp import cron` now emits crontab-wide settings per task instead of daemon-wide.** A crontab's `SHELL`, `CRON_TZ`/`TZ`, and top-of-file `VAR=val` lines become each imported task's own `shell`, `timezone`, and `env` rather than `[defaults]` and `[scheduler] timezone`. Importing keeps its hands off your global settings, two crontabs with different values no longer overwrite each other, and the result is safe to live in an included file. See [Migrating from cron](https://docs.runwisp.com/recipes/migrating-from-cron/#how-the-pieces-map).

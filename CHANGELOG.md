...
- **Notifications subsystem.** RunWisp now pushes failures instead of waiting for you to look. **In-app notifications are on by default** — every failed, timed-out, or crashed run lights up the bell in the Web UI and the footer in the TUI with zero config required. The feed coalesces repeats, draws a sparkline of recent occurrences, and renders rhythm-aware phrases ("12× in the last hour, latest 30s ago"). Each notification carries its own read state: opening the failed run (clicking it in the Web UI, or pressing `enter` in the TUI panel) acknowledges that one row, the Web UI's "Mark all read" clears the badge in bulk, and a fresh occurrence of the same failure pops the row back to unread. The Web UI gains a bell + popover in the topbar and a `/notifications` page; the TUI mirrors it with a collapsible panel on the home screen — press `n` to expand, `j`/`k` to navigate, `enter` to open the run, and `r` to toggle the cursor row's read state. For outbound channels, configure `[[notifier]]` blocks for Slack and Telegram, declare `[[notification_route]]` rules (or use the per-task `notify_on_failure = [...]` shorthand), and failures flow out as messages alongside the in-app feed. Outbound delivery failures surface as in-app notifications too, so a broken Slack webhook never goes unnoticed. Secrets follow `*_env` > `*_file` > inline precedence; `inapp` is reserved as a notifier id. Disable the in-app surface with `[notify] disable_inapp = true`. The outbound retry budget can also be shortened globally via the `RUNWISP_NOTIFY_MAX_ELAPSED` env var (default 5 min) — handy for tests or operators who want delivery failures to surface in seconds.

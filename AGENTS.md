...
4. **User-facing changes** require a `CHANGELOG.md` entry. Keep entries short — one or two sentences naming what changed, plus a docs link for the "how". Don't explain usage in the changelog itself; readers can follow the link. The changelog is for **product-visible changes only** — never log docs edits, README/site copy, internal refactors, test or CI changes, or other work the user can't observe in the product.
